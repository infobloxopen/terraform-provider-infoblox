...
var (
	_ list.ListResource                   = &NetworkListList{}
	_ list.ListResourceWithConfigure      = &NetworkListList{}
	_ list.ListResourceWithValidateConfig = &NetworkListList{}
)

func NewNetworkListList() list.ListResource {
	return &NetworkListList{}
}
...
type NetworkListListModel struct {
	Filters types.Map `tfsdk:"filters"`
}

func (l *NetworkListList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_network_list"
}

func (l *NetworkListList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewNetworkListService(l.backend, client.NIOS, client.UDDI)
}
...
func (l *NetworkListList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data NetworkListListModel
...
func (l *NetworkListList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data NetworkListListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_network_list list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.NetworkList
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.NetworkList, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.NetworkList, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list NetworkList records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
