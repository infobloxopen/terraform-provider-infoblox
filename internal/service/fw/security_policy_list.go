...
var (
	_ list.ListResource                   = &SecurityPolicyList{}
	_ list.ListResourceWithConfigure      = &SecurityPolicyList{}
	_ list.ListResourceWithValidateConfig = &SecurityPolicyList{}
)

func NewSecurityPolicyList() list.ListResource {
...
type SecurityPolicyListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *SecurityPolicyList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_security_policy"
}

func (l *SecurityPolicyList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewSecurityPolicyService(l.backend, client.NIOS, client.UDDI)
}

func (l *SecurityPolicyList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox SecurityPolicy from the UDDI backend.",
...
func (l *SecurityPolicyList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data SecurityPolicyListModel
...
func (l *SecurityPolicyList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data SecurityPolicyListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_security_policy list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.SecurityPolicy
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.SecurityPolicy, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.SecurityPolicy, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list SecurityPolicy records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
