...
var (
	_ list.ListResource                   = &ApplicationFilterList{}
	_ list.ListResourceWithConfigure      = &ApplicationFilterList{}
	_ list.ListResourceWithValidateConfig = &ApplicationFilterList{}
)

func NewApplicationFilterList() list.ListResource {
	return &ApplicationFilterList{}
}

type ApplicationFilterList struct {
	backend core.BackendType
	service coresvc.ApplicationFilterService
}

type ApplicationFilterListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *ApplicationFilterList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_application_filter"
}

func (l *ApplicationFilterList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewApplicationFilterService(l.backend, client.NIOS, client.UDDI)
}

func (l *ApplicationFilterList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox ApplicationFilter from the UDDI backend.",
...
func (l *ApplicationFilterList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data ApplicationFilterListModel
...
func (l *ApplicationFilterList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data ApplicationFilterListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_application_filter list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.ApplicationFilter
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.ApplicationFilter, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.ApplicationFilter, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list ApplicationFilter records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
