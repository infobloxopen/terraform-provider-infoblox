...
var (
	_ list.ListResource                   = &NamedListList{}
	_ list.ListResourceWithConfigure      = &NamedListList{}
	_ list.ListResourceWithValidateConfig = &NamedListList{}
)

func NewNamedListList() list.ListResource {
	return &NamedListList{}
}

type NamedListList struct {
	backend core.BackendType
	service coresvc.NamedListService
}

type NamedListListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *NamedListList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_named_list"
}

func (l *NamedListList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewNamedListService(l.backend, client.NIOS, client.UDDI)
}

func (l *NamedListList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox NamedList from the UDDI backend. Named lists are also known as custom lists.",
...
func (l *NamedListList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data NamedListListModel
...
func (l *NamedListList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data NamedListListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_named_list list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.NamedList
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.NamedList, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.NamedList, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list NamedList records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
