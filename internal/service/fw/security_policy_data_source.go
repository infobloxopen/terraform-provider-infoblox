...
var _ datasource.DataSource = &SecurityPolicyDataSource{}
var _ datasource.DataSourceWithValidateConfig = &SecurityPolicyDataSource{}
var _ datasource.DataSourceWithConfigure = &SecurityPolicyDataSource{}
...
type SecurityPolicyDataSource struct {
	...
	service coresvc.SecurityPolicyService
}
...
// SecurityPolicyDataSourceModel is the filter model for the datasource
...
func (m *SecurityPolicyDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.SecurityPolicy, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: SecurityPolicyAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, SecurityPolicyAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: SecurityPolicyAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}
...
func (d *SecurityPolicyDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox SecurityPolicy from the UDDI backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(SecurityPolicyResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *SecurityPolicyDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewSecurityPolicyService(d.backend, client.NIOS, client.UDDI)
}

func (d *SecurityPolicyDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data SecurityPolicyDataSourceModel
...
func (d *SecurityPolicyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		ReturnFields: SecurityPolicyReturnFields,
		Paging:       1,
	}
	...
	var allResults []*coremodel.SecurityPolicy
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.SecurityPolicy, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.SecurityPolicy, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list SecurityPolicy records: %s", err))
		return
	}
