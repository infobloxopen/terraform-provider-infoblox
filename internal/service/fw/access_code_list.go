...
var (
	_ list.ListResource                   = &AccessCodeList{}
	_ list.ListResourceWithConfigure      = &AccessCodeList{}
	_ list.ListResourceWithValidateConfig = &AccessCodeList{}
)

func NewAccessCodeList() list.ListResource {
...
type AccessCodeListModel struct {
	Filters types.Map `tfsdk:"filters"`
}

func (l *AccessCodeList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_access_code"
}

func (l *AccessCodeList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewAccessCodeService(l.backend, client.NIOS, client.UDDI)
}

func (l *AccessCodeList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox AccessCode from the UDDI backend.",
...
func (l *AccessCodeList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data AccessCodeListModel
...
func (l *AccessCodeList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data AccessCodeListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_access_code list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.AccessCode
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.AccessCode, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.AccessCode, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list AccessCode records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
