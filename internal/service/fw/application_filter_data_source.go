...
var _ datasource.DataSource = &ApplicationFilterDataSource{}
var _ datasource.DataSourceWithValidateConfig = &ApplicationFilterDataSource{}
var _ datasource.DataSourceWithConfigure = &ApplicationFilterDataSource{}
...
// ApplicationFilterDataSourceModel is the filter model for the datasource
type ApplicationFilterDataSourceModel struct {
...
func (m *ApplicationFilterDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.ApplicationFilter, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: ApplicationFilterAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, ApplicationFilterAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: ApplicationFilterAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}
...
func (d *ApplicationFilterDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox ApplicationFilter from the UDDI backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(ApplicationFilterResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *ApplicationFilterDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewApplicationFilterService(d.backend, client.NIOS, client.UDDI)
}

func (d *ApplicationFilterDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data ApplicationFilterDataSourceModel
...
func (d *ApplicationFilterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.ApplicationFilter
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.ApplicationFilter, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.ApplicationFilter, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list ApplicationFilter records: %s", err))
		return
	}
