...
var (
	_ list.ListResource                   = &FederatedRealmList{}
	_ list.ListResourceWithConfigure      = &FederatedRealmList{}
	_ list.ListResourceWithValidateConfig = &FederatedRealmList{}
)

func NewFederatedRealmList() list.ListResource {
	return &FederatedRealmList{}
}

type FederatedRealmList struct {
	backend core.BackendType
	service coresvc.FederatedRealmService
}

type FederatedRealmListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *FederatedRealmList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_federated_realm"
}

func (l *FederatedRealmList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewFederatedRealmService(l.backend, client.NIOS, client.UDDI)
}

func (l *FederatedRealmList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox FederatedRealm from the UDDI backend.",
...
func (l *FederatedRealmList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data FederatedRealmListModel
...
func (l *FederatedRealmList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data FederatedRealmListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_federated_realm list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.FederatedRealm
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.FederatedRealm, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.FederatedRealm, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list FederatedRealm records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
