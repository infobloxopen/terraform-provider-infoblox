...
var (
	_ list.ListResource                   = &NamedaclList{}
	_ list.ListResourceWithConfigure      = &NamedaclList{}
	_ list.ListResourceWithValidateConfig = &NamedaclList{}
)

func NewNamedaclList() list.ListResource {
	return &NamedaclList{}
}

type NamedaclList struct {
	backend core.BackendType
	service coresvc.NamedaclService
}

type NamedaclListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
	TagFilters     types.Map `tfsdk:"tag_filters"`
}

func (l *NamedaclList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_namedacl"
}

func (l *NamedaclList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewNamedaclService(l.backend, client.NIOS, client.UDDI)
}

func (l *NamedaclList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Namedacl from both the NIOS and UDDI backends.",
...
func (l *NamedaclList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data NamedaclListModel
...
func (l *NamedaclList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data NamedaclListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_namedacl list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.Namedacl
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Namedacl, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Namedacl, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Namedacl records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
