...
var _ datasource.DataSourceWithValidateConfig = &DtcLbdnDataSource{}
var _ datasource.DataSourceWithConfigure = &DtcLbdnDataSource{}
...
func (d *DtcLbdnDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_lbdn"
}

// DtcLbdnDataSourceModel is the filter model for the datasource
type DtcLbdnDataSourceModel struct {
...
func (m *DtcLbdnDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.DtcLbdn, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: DtcLbdnAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, DtcLbdnAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: DtcLbdnAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *DtcLbdnDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox DtcLbdn from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(DtcLbdnResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *DtcLbdnDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewDtcLbdnService(d.backend, client.NIOS, client.UDDI)
}

func (d *DtcLbdnDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data DtcLbdnDataSourceModel
...
func (d *DtcLbdnDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.DtcLbdn
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.DtcLbdn, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.DtcLbdn, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list DtcLbdn records: %s", err))
		return
	}
