...
var (
	_ list.ListResource                   = &DtcMonitorTcpList{}
	_ list.ListResourceWithConfigure      = &DtcMonitorTcpList{}
	_ list.ListResourceWithValidateConfig = &DtcMonitorTcpList{}
)

func NewDtcMonitorTcpList() list.ListResource {
	return &DtcMonitorTcpList{}
}

type DtcMonitorTcpList struct {
	backend core.BackendType
	service coresvc.DtcMonitorTcpService
}

type DtcMonitorTcpListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
	TagFilters     types.Map `tfsdk:"tag_filters"`
}

func (l *DtcMonitorTcpList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_monitor_tcp"
}

func (l *DtcMonitorTcpList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDtcMonitorTcpService(l.backend, client.NIOS, client.UDDI)
}

func (l *DtcMonitorTcpList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox DtcMonitorTcp from both the NIOS and UDDI backends.",
...
func (l *DtcMonitorTcpList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DtcMonitorTcpListModel
...
func (l *DtcMonitorTcpList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data DtcMonitorTcpListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dtc_monitor_tcp list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.DtcMonitorTcp
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DtcMonitorTcp, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DtcMonitorTcp, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DtcMonitorTcp records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
