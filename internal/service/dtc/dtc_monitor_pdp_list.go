...
var (
	_ list.ListResource                   = &DtcMonitorPdpList{}
	_ list.ListResourceWithConfigure      = &DtcMonitorPdpList{}
	_ list.ListResourceWithValidateConfig = &DtcMonitorPdpList{}
)

func NewDtcMonitorPdpList() list.ListResource {
	return &DtcMonitorPdpList{}
}

type DtcMonitorPdpList struct {
...
func (l *DtcMonitorPdpList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDtcMonitorPdpService(l.backend, client.NIOS, client.UDDI)
}
...
func (l *DtcMonitorPdpList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DtcMonitorPdpListModel
...
func (l *DtcMonitorPdpList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dtc_monitor_pdp list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.DtcMonitorPdp
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DtcMonitorPdp, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DtcMonitorPdp, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DtcMonitorPdp records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
