...
var _ datasource.DataSourceWithValidateConfig = &DtcPoolDataSource{}
var _ datasource.DataSourceWithConfigure = &DtcPoolDataSource{}
...
func (d *DtcPoolDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_pool"
}

// DtcPoolDataSourceModel is the filter model for the datasource
type DtcPoolDataSourceModel struct {
...
func (m *DtcPoolDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.DtcPool, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: DtcPoolAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, DtcPoolAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: DtcPoolAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *DtcPoolDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox DtcPool from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(DtcPoolResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *DtcPoolDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewDtcPoolService(d.backend, client.NIOS, client.UDDI)
}

func (d *DtcPoolDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data DtcPoolDataSourceModel
...
func (d *DtcPoolDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.DtcPool
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.DtcPool, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.DtcPool, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list DtcPool records: %s", err))
		return
	}
