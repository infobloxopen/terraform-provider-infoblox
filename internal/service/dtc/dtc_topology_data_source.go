...
var _ datasource.DataSourceWithValidateConfig = &DtcTopologyDataSource{}
var _ datasource.DataSourceWithConfigure = &DtcTopologyDataSource{}
...
// DtcTopologyDataSourceModel is the filter model for the datasource
...
func (m *DtcTopologyDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.DtcTopology, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: DtcTopologyAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, DtcTopologyAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: DtcTopologyAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(DtcTopologyResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *DtcTopologyDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewDtcTopologyService(d.backend, client.NIOS, client.UDDI)
}

func (d *DtcTopologyDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data DtcTopologyDataSourceModel
...
func (d *DtcTopologyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.DtcTopology
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.DtcTopology, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.DtcTopology, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list DtcTopology records: %s", err))
		return
	}
