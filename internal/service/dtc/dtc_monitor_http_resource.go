...
var (
	_ resource.Resource                   = &DtcMonitorHttpResource{}
	_ resource.ResourceWithValidateConfig = &DtcMonitorHttpResource{}
	_ resource.ResourceWithConfigure      = &DtcMonitorHttpResource{}
	_ resource.ResourceWithImportState    = &DtcMonitorHttpResource{}
	_ resource.ResourceWithIdentity       = &DtcMonitorHttpResource{}
)
...
type DtcMonitorHttpResource struct {
	...
	service coresvc.DtcMonitorHttpService
}

func (r *DtcMonitorHttpResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_monitor_http"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *DtcMonitorHttpResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *DtcMonitorHttpResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox DtcMonitorHttp in both NIOS and UDDI backends.",
		Attributes:          DtcMonitorHttpResourceSchemaAttributes,
	}
}

func (r *DtcMonitorHttpResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewDtcMonitorHttpService(r.backend, client.NIOS, client.UDDI)
}

func (r *DtcMonitorHttpResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.DtcMonitorHttp](r.backend, op)
}
...
func (r *DtcMonitorHttpResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateDtcMonitorHttp(ctx, data, resp)
}
...
func (r *DtcMonitorHttpResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSDtcMonitorHttpModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSDtcMonitorHttpModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSDtcMonitorHttpAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.DtcMonitorHttp
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: DtcMonitorHttpReturnFields,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create DtcMonitorHttp: %s", err))
		return
	}
...
func (r *DtcMonitorHttpResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read DtcMonitorHttp: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorHttpModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read DtcMonitorHttp because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *DtcMonitorHttpResource) ReadByExtAttrs(ctx context.Context, data *DtcMonitorHttpModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.DtcMonitorHttp
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search DtcMonitorHttp by extattrs: %s", err))
		return true
	}
...
func (r *DtcMonitorHttpResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSDtcMonitorHttpModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorHttpModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSDtcMonitorHttpModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSDtcMonitorHttpAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update DtcMonitorHttp: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSDtcMonitorHttpModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSDtcMonitorHttpAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *DtcMonitorHttpResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete DtcMonitorHttp: %s", err))
	}
}
