...
var (
	_ list.ListResource                   = &DtcMonitorIcmpList{}
	_ list.ListResourceWithConfigure      = &DtcMonitorIcmpList{}
	_ list.ListResourceWithValidateConfig = &DtcMonitorIcmpList{}
)

func NewDtcMonitorIcmpList() list.ListResource {
	return &DtcMonitorIcmpList{}
}

type DtcMonitorIcmpList struct {
	backend core.BackendType
	service coresvc.DtcMonitorIcmpService
}

type DtcMonitorIcmpListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
	TagFilters     types.Map `tfsdk:"tag_filters"`
}

func (l *DtcMonitorIcmpList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_monitor_icmp"
}

func (l *DtcMonitorIcmpList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDtcMonitorIcmpService(l.backend, client.NIOS, client.UDDI)
}

func (l *DtcMonitorIcmpList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox DtcMonitorIcmp from both the NIOS and UDDI backends.",
...
func (l *DtcMonitorIcmpList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DtcMonitorIcmpListModel
...
func (l *DtcMonitorIcmpList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data DtcMonitorIcmpListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dtc_monitor_icmp list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.DtcMonitorIcmp
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DtcMonitorIcmp, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DtcMonitorIcmp, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DtcMonitorIcmp records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
