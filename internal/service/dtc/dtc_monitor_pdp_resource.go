...
var (
	_ resource.Resource                   = &DtcMonitorPdpResource{}
	_ resource.ResourceWithValidateConfig = &DtcMonitorPdpResource{}
	_ resource.ResourceWithConfigure      = &DtcMonitorPdpResource{}
	_ resource.ResourceWithImportState    = &DtcMonitorPdpResource{}
	_ resource.ResourceWithIdentity       = &DtcMonitorPdpResource{}
)
...
type DtcMonitorPdpResource struct {
	...
	service coresvc.DtcMonitorPdpService
}

func (r *DtcMonitorPdpResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_monitor_pdp"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *DtcMonitorPdpResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *DtcMonitorPdpResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox DtcMonitorPdp in both NIOS and UDDI backends.",
		Attributes:          DtcMonitorPdpResourceSchemaAttributes,
	}
}

func (r *DtcMonitorPdpResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewDtcMonitorPdpService(r.backend, client.NIOS, client.UDDI)
}

func (r *DtcMonitorPdpResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.DtcMonitorPdp](r.backend, op)
}

func (r *DtcMonitorPdpResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateDtcMonitorPdp(ctx, data, resp)
}
...
func (r *DtcMonitorPdpResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSDtcMonitorPdpModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSDtcMonitorPdpModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSDtcMonitorPdpAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.DtcMonitorPdp
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create DtcMonitorPdp: %s", err))
		return
	}
...
func (r *DtcMonitorPdpResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read DtcMonitorPdp: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorPdpModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read DtcMonitorPdp because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *DtcMonitorPdpResource) ReadByExtAttrs(ctx context.Context, data *DtcMonitorPdpModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.DtcMonitorPdp
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search DtcMonitorPdp by extattrs: %s", err))
		return true
	}
...
func (r *DtcMonitorPdpResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSDtcMonitorPdpModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorPdpModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSDtcMonitorPdpModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSDtcMonitorPdpAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update DtcMonitorPdp: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSDtcMonitorPdpModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSDtcMonitorPdpAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *DtcMonitorPdpResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete DtcMonitorPdp: %s", err))
	}
}
