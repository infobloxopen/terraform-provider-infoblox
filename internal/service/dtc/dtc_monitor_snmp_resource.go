...
var (
	_ resource.Resource                   = &DtcMonitorSnmpResource{}
	_ resource.ResourceWithValidateConfig = &DtcMonitorSnmpResource{}
	_ resource.ResourceWithConfigure      = &DtcMonitorSnmpResource{}
	_ resource.ResourceWithImportState    = &DtcMonitorSnmpResource{}
	_ resource.ResourceWithIdentity       = &DtcMonitorSnmpResource{}
)
...
type DtcMonitorSnmpResource struct {
	...
	service coresvc.DtcMonitorSnmpService
}

func (r *DtcMonitorSnmpResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_monitor_snmp"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *DtcMonitorSnmpResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *DtcMonitorSnmpResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox DtcMonitorSnmp in both NIOS and UDDI backends.",
		Attributes:          DtcMonitorSnmpResourceSchemaAttributes,
	}
}

func (r *DtcMonitorSnmpResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewDtcMonitorSnmpService(r.backend, client.NIOS, client.UDDI)
}

func (r *DtcMonitorSnmpResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.DtcMonitorSnmp](r.backend, op)
}
...
func (r *DtcMonitorSnmpResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateDtcMonitorSnmp(ctx, data, resp)
}
...
func (r *DtcMonitorSnmpResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSDtcMonitorSnmpModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSDtcMonitorSnmpModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSDtcMonitorSnmpAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.DtcMonitorSnmp
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: DtcMonitorSnmpReturnFields,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create DtcMonitorSnmp: %s", err))
		return
	}
...
func (r *DtcMonitorSnmpResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read DtcMonitorSnmp: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorSnmpModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read DtcMonitorSnmp because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *DtcMonitorSnmpResource) ReadByExtAttrs(ctx context.Context, data *DtcMonitorSnmpModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.DtcMonitorSnmp
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search DtcMonitorSnmp by extattrs: %s", err))
		return true
	}
...
func (r *DtcMonitorSnmpResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSDtcMonitorSnmpModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorSnmpModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSDtcMonitorSnmpModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSDtcMonitorSnmpAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update DtcMonitorSnmp: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSDtcMonitorSnmpModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSDtcMonitorSnmpAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *DtcMonitorSnmpResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete DtcMonitorSnmp: %s", err))
	}
}
