...
var (
	_ list.ListResource                   = &DtcTopologyList{}
	_ list.ListResourceWithConfigure      = &DtcTopologyList{}
	_ list.ListResourceWithValidateConfig = &DtcTopologyList{}
)

func NewDtcTopologyList() list.ListResource {
	return &DtcTopologyList{}
}
...
func (l *DtcTopologyList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDtcTopologyService(l.backend, client.NIOS, client.UDDI)
}
...
func (l *DtcTopologyList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DtcTopologyListModel
...
func (l *DtcTopologyList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dtc_topology list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.DtcTopology
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DtcTopology, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DtcTopology, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DtcTopology records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
