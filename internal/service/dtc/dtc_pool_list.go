...
var (
	_ list.ListResource                   = &DtcPoolList{}
	_ list.ListResourceWithConfigure      = &DtcPoolList{}
	_ list.ListResourceWithValidateConfig = &DtcPoolList{}
)

func NewDtcPoolList() list.ListResource {
	return &DtcPoolList{}
}
...
type DtcPoolListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
	TagFilters     types.Map `tfsdk:"tag_filters"`
}

func (l *DtcPoolList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_pool"
}

func (l *DtcPoolList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDtcPoolService(l.backend, client.NIOS, client.UDDI)
}

func (l *DtcPoolList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox DtcPool from both the NIOS and UDDI backends.",
...
func (l *DtcPoolList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DtcPoolListModel
...
func (l *DtcPoolList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data DtcPoolListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dtc_pool list: req.Limit=%d backend=%s includeResource=%t",
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DtcPool, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DtcPool, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DtcPool records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
