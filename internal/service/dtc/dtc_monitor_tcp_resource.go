...
var (
	_ resource.Resource                   = &DtcMonitorTcpResource{}
	_ resource.ResourceWithValidateConfig = &DtcMonitorTcpResource{}
	_ resource.ResourceWithConfigure      = &DtcMonitorTcpResource{}
	_ resource.ResourceWithImportState    = &DtcMonitorTcpResource{}
	_ resource.ResourceWithIdentity       = &DtcMonitorTcpResource{}
)
...
func (r *DtcMonitorTcpResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (r *DtcMonitorTcpResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *DtcMonitorTcpResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox DtcMonitorTcp in both NIOS and UDDI backends.",
		Attributes:          DtcMonitorTcpResourceSchemaAttributes,
	}
}

func (r *DtcMonitorTcpResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewDtcMonitorTcpService(r.backend, client.NIOS, client.UDDI)
}

func (r *DtcMonitorTcpResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.DtcMonitorTcp](r.backend, op)
}

func (r *DtcMonitorTcpResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateDtcMonitorTcp(ctx, data, resp)
}
...
func (r *DtcMonitorTcpResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSDtcMonitorTcpModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSDtcMonitorTcpModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSDtcMonitorTcpAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.DtcMonitorTcp
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create DtcMonitorTcp: %s", err))
		return
	}
...
func (r *DtcMonitorTcpResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read DtcMonitorTcp: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorTcpModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read DtcMonitorTcp because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *DtcMonitorTcpResource) ReadByExtAttrs(ctx context.Context, data *DtcMonitorTcpModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.DtcMonitorTcp
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search DtcMonitorTcp by extattrs: %s", err))
		return true
	}
...
func (r *DtcMonitorTcpResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSDtcMonitorTcpModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorTcpModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSDtcMonitorTcpModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSDtcMonitorTcpAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update DtcMonitorTcp: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSDtcMonitorTcpModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSDtcMonitorTcpAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *DtcMonitorTcpResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete DtcMonitorTcp: %s", err))
	}
}
