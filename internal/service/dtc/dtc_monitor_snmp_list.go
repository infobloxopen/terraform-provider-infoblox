...
var (
	_ list.ListResource                   = &DtcMonitorSnmpList{}
	_ list.ListResourceWithConfigure      = &DtcMonitorSnmpList{}
	_ list.ListResourceWithValidateConfig = &DtcMonitorSnmpList{}
)

func NewDtcMonitorSnmpList() list.ListResource {
	return &DtcMonitorSnmpList{}
}

type DtcMonitorSnmpList struct {
	backend core.BackendType
	service coresvc.DtcMonitorSnmpService
}

type DtcMonitorSnmpListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
	TagFilters     types.Map `tfsdk:"tag_filters"`
}

func (l *DtcMonitorSnmpList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_monitor_snmp"
}

func (l *DtcMonitorSnmpList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDtcMonitorSnmpService(l.backend, client.NIOS, client.UDDI)
}

func (l *DtcMonitorSnmpList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox DtcMonitorSnmp from both the NIOS and UDDI backends.",
...
func (l *DtcMonitorSnmpList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DtcMonitorSnmpListModel
...
func (l *DtcMonitorSnmpList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data DtcMonitorSnmpListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dtc_monitor_snmp list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.DtcMonitorSnmp
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DtcMonitorSnmp, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DtcMonitorSnmp, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DtcMonitorSnmp records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
