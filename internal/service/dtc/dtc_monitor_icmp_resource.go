...
var (
	_ resource.Resource                   = &DtcMonitorIcmpResource{}
	_ resource.ResourceWithValidateConfig = &DtcMonitorIcmpResource{}
	_ resource.ResourceWithConfigure      = &DtcMonitorIcmpResource{}
	_ resource.ResourceWithImportState    = &DtcMonitorIcmpResource{}
	_ resource.ResourceWithIdentity       = &DtcMonitorIcmpResource{}
)
...
type DtcMonitorIcmpResource struct {
	...
	service coresvc.DtcMonitorIcmpService
}

func (r *DtcMonitorIcmpResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_monitor_icmp"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *DtcMonitorIcmpResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *DtcMonitorIcmpResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox DtcMonitorIcmp in both NIOS and UDDI backends.",
		Attributes:          DtcMonitorIcmpResourceSchemaAttributes,
	}
}

func (r *DtcMonitorIcmpResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewDtcMonitorIcmpService(r.backend, client.NIOS, client.UDDI)
}

func (r *DtcMonitorIcmpResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.DtcMonitorIcmp](r.backend, op)
}

func (r *DtcMonitorIcmpResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateDtcMonitorIcmp(ctx, data, resp)
}
...
func (r *DtcMonitorIcmpResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSDtcMonitorIcmpModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSDtcMonitorIcmpModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSDtcMonitorIcmpAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.DtcMonitorIcmp
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: DtcMonitorIcmpReturnFields,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create DtcMonitorIcmp: %s", err))
		return
	}
...
func (r *DtcMonitorIcmpResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read DtcMonitorIcmp: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorIcmpModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read DtcMonitorIcmp because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *DtcMonitorIcmpResource) ReadByExtAttrs(ctx context.Context, data *DtcMonitorIcmpModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.DtcMonitorIcmp
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search DtcMonitorIcmp by extattrs: %s", err))
		return true
	}
...
func (r *DtcMonitorIcmpResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSDtcMonitorIcmpModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSDtcMonitorIcmpModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSDtcMonitorIcmpModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSDtcMonitorIcmpAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update DtcMonitorIcmp: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSDtcMonitorIcmpModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSDtcMonitorIcmpAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *DtcMonitorIcmpResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete DtcMonitorIcmp: %s", err))
	}
}
