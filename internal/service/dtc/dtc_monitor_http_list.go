...
var (
	_ list.ListResource                   = &DtcMonitorHttpList{}
	_ list.ListResourceWithConfigure      = &DtcMonitorHttpList{}
	_ list.ListResourceWithValidateConfig = &DtcMonitorHttpList{}
)

func NewDtcMonitorHttpList() list.ListResource {
	return &DtcMonitorHttpList{}
}

type DtcMonitorHttpList struct {
	backend core.BackendType
	service coresvc.DtcMonitorHttpService
}

type DtcMonitorHttpListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
	TagFilters     types.Map `tfsdk:"tag_filters"`
}

func (l *DtcMonitorHttpList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dtc_monitor_http"
}

func (l *DtcMonitorHttpList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDtcMonitorHttpService(l.backend, client.NIOS, client.UDDI)
}

func (l *DtcMonitorHttpList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox DtcMonitorHttp from both the NIOS and UDDI backends.",
...
func (l *DtcMonitorHttpList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DtcMonitorHttpListModel
...
func (l *DtcMonitorHttpList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data DtcMonitorHttpListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dtc_monitor_http list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.DtcMonitorHttp
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DtcMonitorHttp, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DtcMonitorHttp, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DtcMonitorHttp records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
