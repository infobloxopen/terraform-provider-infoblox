...
var _ datasource.DataSourceWithValidateConfig = &AnycastConfigDataSource{}
var _ datasource.DataSourceWithConfigure = &AnycastConfigDataSource{}
...
func (d *AnycastConfigDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_anycast_config"
}

// AnycastConfigDataSourceModel is the filter model for the datasource
type AnycastConfigDataSourceModel struct {
...
func (m *AnycastConfigDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.AnycastConfig, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: AnycastConfigAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &AnycastConfigModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, AnycastConfigAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: AnycastConfigAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *AnycastConfigDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox AnycastConfig from the UDDI backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(AnycastConfigResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *AnycastConfigDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewAnycastConfigService(d.backend, client.NIOS, client.UDDI)
}

func (d *AnycastConfigDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data AnycastConfigDataSourceModel
...
func (d *AnycastConfigDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.AnycastConfig
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.AnycastConfig, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.AnycastConfig, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list AnycastConfig records: %s", err))
		return
	}
