...
var (
	_ list.ListResource                   = &AnycastConfigList{}
	_ list.ListResourceWithConfigure      = &AnycastConfigList{}
	_ list.ListResourceWithValidateConfig = &AnycastConfigList{}
)

func NewAnycastConfigList() list.ListResource {
	return &AnycastConfigList{}
}

type AnycastConfigList struct {
	backend core.BackendType
	service coresvc.AnycastConfigService
}

type AnycastConfigListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *AnycastConfigList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_anycast_config"
}

func (l *AnycastConfigList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewAnycastConfigService(l.backend, client.NIOS, client.UDDI)
}

func (l *AnycastConfigList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox AnycastConfig from the UDDI backend.",
...
func (l *AnycastConfigList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data AnycastConfigListModel
...
func (l *AnycastConfigList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data AnycastConfigListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_anycast_config list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.AnycastConfig
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.AnycastConfig, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.AnycastConfig, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list AnycastConfig records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
