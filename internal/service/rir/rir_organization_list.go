...
var (
	_ list.ListResource                   = &RirOrganizationList{}
	_ list.ListResourceWithConfigure      = &RirOrganizationList{}
	_ list.ListResourceWithValidateConfig = &RirOrganizationList{}
)

func NewRirOrganizationList() list.ListResource {
	return &RirOrganizationList{}
}

type RirOrganizationList struct {
	backend core.BackendType
	service coresvc.RirOrganizationService
}

type RirOrganizationListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
}

func (l *RirOrganizationList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_rir_organization"
}

func (l *RirOrganizationList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRirOrganizationService(l.backend, client.NIOS, client.UDDI)
}

func (l *RirOrganizationList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Lists Infoblox RIR Organizations in the NIOS backend.",
...
func (l *RirOrganizationList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data RirOrganizationListModel
...
func (l *RirOrganizationList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RirOrganizationListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_rir_organization list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RirOrganization
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RirOrganization, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RirOrganization, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RirOrganization records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
