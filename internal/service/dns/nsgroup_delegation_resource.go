...
var (
	_ resource.Resource                   = &NsgroupDelegationResource{}
	_ resource.ResourceWithValidateConfig = &NsgroupDelegationResource{}
	_ resource.ResourceWithConfigure      = &NsgroupDelegationResource{}
	_ resource.ResourceWithImportState    = &NsgroupDelegationResource{}
	_ resource.ResourceWithIdentity       = &NsgroupDelegationResource{}
)
...
type NsgroupDelegationResource struct {
	...
	service coresvc.NsgroupDelegationService
}

func (r *NsgroupDelegationResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_nsgroup_delegation"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *NsgroupDelegationResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *NsgroupDelegationResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox NsgroupDelegation in the NIOS backend.",
		Attributes:          NsgroupDelegationResourceSchemaAttributes,
	}
}

func (r *NsgroupDelegationResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewNsgroupDelegationService(r.backend, client.NIOS, client.UDDI)
}

func (r *NsgroupDelegationResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.NsgroupDelegation](r.backend, op)
}

func (r *NsgroupDelegationResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateNsgroupDelegation(ctx, data, resp)
}
...
func (r *NsgroupDelegationResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSNsgroupDelegationModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSNsgroupDelegationModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSNsgroupDelegationAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.NsgroupDelegation
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: NsgroupDelegationReturnFields,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create NsgroupDelegation: %s", err))
		return
	}
...
func (r *NsgroupDelegationResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read NsgroupDelegation: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSNsgroupDelegationModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read NsgroupDelegation because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *NsgroupDelegationResource) ReadByExtAttrs(ctx context.Context, data *NsgroupDelegationModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.NsgroupDelegation
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search NsgroupDelegation by extattrs: %s", err))
		return true
	}
...
func (r *NsgroupDelegationResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSNsgroupDelegationModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSNsgroupDelegationModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSNsgroupDelegationModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSNsgroupDelegationAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update NsgroupDelegation: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSNsgroupDelegationModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSNsgroupDelegationAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *NsgroupDelegationResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete NsgroupDelegation: %s", err))
	}
}
