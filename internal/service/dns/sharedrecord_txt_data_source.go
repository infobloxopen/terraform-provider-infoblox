...
var _ datasource.DataSourceWithValidateConfig = &SharedrecordTxtDataSource{}
var _ datasource.DataSourceWithConfigure = &SharedrecordTxtDataSource{}
...
func (d *SharedrecordTxtDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_sharedrecord_txt"
}

// SharedrecordTxtDataSourceModel is the filter model for the datasource
type SharedrecordTxtDataSourceModel struct {
...
func (m *SharedrecordTxtDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.SharedrecordTxt, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: SharedrecordTxtAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &SharedrecordTxtModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, SharedrecordTxtAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: SharedrecordTxtAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *SharedrecordTxtDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox SharedrecordTxt from the NIOS backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(SharedrecordTxtResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *SharedrecordTxtDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewSharedrecordTxtService(d.backend, client.NIOS, client.UDDI)
}

func (d *SharedrecordTxtDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data SharedrecordTxtDataSourceModel
...
func (d *SharedrecordTxtDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.SharedrecordTxt
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.SharedrecordTxt, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.SharedrecordTxt, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list SharedrecordTxt records: %s", err))
		return
	}
