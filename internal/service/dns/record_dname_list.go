...
func (l *RecordDnameList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_dname"
}

func (l *RecordDnameList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordDnameService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordDnameList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordDname from the configured backend (NIOS or UDDI).",
...
func (l *RecordDnameList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_dname list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordDname
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordDname, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordDname, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordDname records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
