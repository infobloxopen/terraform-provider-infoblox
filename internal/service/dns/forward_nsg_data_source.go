...
var _ datasource.DataSource = &ForwardNsgDataSource{}
var _ datasource.DataSourceWithValidateConfig = &ForwardNsgDataSource{}
var _ datasource.DataSourceWithConfigure = &ForwardNsgDataSource{}

func NewForwardNsgDataSource() datasource.DataSource {
	return &ForwardNsgDataSource{}
}
...
// ForwardNsgDataSourceModel is the filter model for the datasource
type ForwardNsgDataSourceModel struct {
...
func (m *ForwardNsgDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.ForwardNsg, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: ForwardNsgAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &ForwardNsgModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, ForwardNsgAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: ForwardNsgAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}
...
func (d *ForwardNsgDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox ForwardNsg from the UDDI backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(ForwardNsgResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *ForwardNsgDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewForwardNsgService(d.backend, client.NIOS, client.UDDI)
}

func (d *ForwardNsgDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data ForwardNsgDataSourceModel
...
func (d *ForwardNsgDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.ForwardNsg
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.ForwardNsg, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.ForwardNsg, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list ForwardNsg records: %s", err))
		return
	}
