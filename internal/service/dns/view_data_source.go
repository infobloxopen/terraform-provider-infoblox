...
var _ datasource.DataSourceWithValidateConfig = &ViewDataSource{}
...
// ViewDataSourceModel is the filter model for the datasource
...
func (m *ViewDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.View, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: ViewAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, ViewAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: ViewAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}
		...
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(ViewResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *ViewDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewViewService(d.backend, client.NIOS, client.UDDI)
}

func (d *ViewDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
...
func (d *ViewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.View
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.View, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list View records: %s", err))
		return
	}
