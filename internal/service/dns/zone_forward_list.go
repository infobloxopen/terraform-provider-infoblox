...
var (
	_ list.ListResource                   = &ZoneForwardList{}
	_ list.ListResourceWithConfigure      = &ZoneForwardList{}
	_ list.ListResourceWithValidateConfig = &ZoneForwardList{}
)
...
func (l *ZoneForwardList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_zone_forward"
}

func (l *ZoneForwardList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewZoneForwardService(l.backend, client.NIOS, client.UDDI)
}

func (l *ZoneForwardList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox ZoneForward from both the NIOS and UDDI backends.",
...
func (l *ZoneForwardList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
...
func (l *ZoneForwardList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_zone_forward list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.ZoneForward
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.ZoneForward, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.ZoneForward, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list ZoneForward records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
