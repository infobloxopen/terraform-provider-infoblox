...
var (
	_ resource.Resource                   = &NsgroupForwardingmemberResource{}
	_ resource.ResourceWithValidateConfig = &NsgroupForwardingmemberResource{}
	_ resource.ResourceWithConfigure      = &NsgroupForwardingmemberResource{}
	_ resource.ResourceWithImportState    = &NsgroupForwardingmemberResource{}
	_ resource.ResourceWithIdentity       = &NsgroupForwardingmemberResource{}
)
...
func (r *NsgroupForwardingmemberResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (r *NsgroupForwardingmemberResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *NsgroupForwardingmemberResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox NsgroupForwardingmember in the NIOS backend.",
		Attributes:          NsgroupForwardingmemberResourceSchemaAttributes,
	}
}

func (r *NsgroupForwardingmemberResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewNsgroupForwardingmemberService(r.backend, client.NIOS, client.UDDI)
}

func (r *NsgroupForwardingmemberResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.NsgroupForwardingmember](r.backend, op)
}

func (r *NsgroupForwardingmemberResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateNsgroupForwardingmember(ctx, data, resp)
}
...
func (r *NsgroupForwardingmemberResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSNsgroupForwardingmemberModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSNsgroupForwardingmemberModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSNsgroupForwardingmemberAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.NsgroupForwardingmember
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create NsgroupForwardingmember: %s", err))
		return
	}
...
func (r *NsgroupForwardingmemberResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read NsgroupForwardingmember: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSNsgroupForwardingmemberModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read NsgroupForwardingmember because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *NsgroupForwardingmemberResource) ReadByExtAttrs(ctx context.Context, data *NsgroupForwardingmemberModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.NsgroupForwardingmember
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search NsgroupForwardingmember by extattrs: %s", err))
		return true
	}
...
func (r *NsgroupForwardingmemberResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSNsgroupForwardingmemberModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSNsgroupForwardingmemberModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSNsgroupForwardingmemberModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSNsgroupForwardingmemberAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update NsgroupForwardingmember: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSNsgroupForwardingmemberModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSNsgroupForwardingmemberAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *NsgroupForwardingmemberResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete NsgroupForwardingmember: %s", err))
	}
}
