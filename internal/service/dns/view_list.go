...
func (l *ViewList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_view"
}

func (l *ViewList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewViewService(l.backend, client.NIOS, client.UDDI)
}

func (l *ViewList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox View from the configured backend (NIOS or UDDI).",
...
func (l *ViewList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_view list: req.Limit=%d backend=%s includeResource=%t",
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.View, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.View, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list View records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
