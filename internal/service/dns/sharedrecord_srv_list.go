...
var (
	_ list.ListResource                   = &SharedrecordSrvList{}
	_ list.ListResourceWithConfigure      = &SharedrecordSrvList{}
	_ list.ListResourceWithValidateConfig = &SharedrecordSrvList{}
)
...
type SharedrecordSrvList struct {
	...
	service coresvc.SharedrecordSrvService
}
...
func (l *SharedrecordSrvList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_sharedrecord_srv"
}

func (l *SharedrecordSrvList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewSharedrecordSrvService(l.backend, client.NIOS, client.UDDI)
}

func (l *SharedrecordSrvList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox SharedrecordSrv from the NIOS backend.",
...
func (l *SharedrecordSrvList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
...
func (l *SharedrecordSrvList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_sharedrecord_srv list: req.Limit=%d backend=%s includeResource=%t",
	...
	opts := &core.ListOptions{
		...
		ReturnFields:  SharedrecordSrvReturnFields,
		Paging:        1,
	}
	if diags.HasError() {
		stream.Results = list.ListResultsStreamDiagnostics(diags)
	...
	var records []*coremodel.SharedrecordSrv
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.SharedrecordSrv, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.SharedrecordSrv, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list SharedrecordSrv records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
