...
func (l *RecordNaptrList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_naptr"
}

func (l *RecordNaptrList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordNaptrService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordNaptrList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordNaptr from the configured backend (NIOS or UDDI).",
...
func (l *RecordNaptrList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_naptr list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordNaptr
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordNaptr, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordNaptr, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordNaptr records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
