...
var (
	_ list.ListResource                   = &RecordMxList{}
	_ list.ListResourceWithConfigure      = &RecordMxList{}
	_ list.ListResourceWithValidateConfig = &RecordMxList{}
)
...
func (l *RecordMxList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_mx"
}

func (l *RecordMxList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordMxService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordMxList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordMx from both the NIOS and UDDI backends.",
...
func (l *RecordMxList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
...
func (l *RecordMxList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_mx list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordMx
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordMx, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordMx, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordMx records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
