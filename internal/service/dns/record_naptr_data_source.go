...
var _ datasource.DataSourceWithValidateConfig = &RecordNaptrDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordNaptrDataSource{}
...
func (d *RecordNaptrDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_naptr"
}

// RecordNaptrDataSourceModel is the filter model for the datasource
type RecordNaptrDataSourceModel struct {
...
func (m *RecordNaptrDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordNaptr, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordNaptrAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordNaptrModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordNaptrAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordNaptrAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordNaptrDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordNaptr from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordNaptrResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordNaptrDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordNaptrService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordNaptrDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordNaptrDataSourceModel
...
func (d *RecordNaptrDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": RecordNaptrType},
	...
	var allResults []*coremodel.RecordNaptr
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordNaptr, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordNaptr records: %s", err))
		return
	}
