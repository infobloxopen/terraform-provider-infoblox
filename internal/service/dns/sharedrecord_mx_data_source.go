...
var _ datasource.DataSourceWithValidateConfig = &SharedrecordMxDataSource{}
var _ datasource.DataSourceWithConfigure = &SharedrecordMxDataSource{}
...
func (d *SharedrecordMxDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_sharedrecord_mx"
}

// SharedrecordMxDataSourceModel is the filter model for the datasource
type SharedrecordMxDataSourceModel struct {
...
func (m *SharedrecordMxDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.SharedrecordMx, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: SharedrecordMxAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &SharedrecordMxModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, SharedrecordMxAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: SharedrecordMxAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *SharedrecordMxDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox SharedrecordMx from the NIOS backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(SharedrecordMxResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *SharedrecordMxDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewSharedrecordMxService(d.backend, client.NIOS, client.UDDI)
}

func (d *SharedrecordMxDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data SharedrecordMxDataSourceModel
...
func (d *SharedrecordMxDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.SharedrecordMx
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.SharedrecordMx, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.SharedrecordMx, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list SharedrecordMx records: %s", err))
		return
	}
