...
var _ datasource.DataSourceWithValidateConfig = &RecordTxtDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordTxtDataSource{}
...
func (d *RecordTxtDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_txt"
}

// RecordTxtDataSourceModel is the filter model for the datasource
type RecordTxtDataSourceModel struct {
...
func (m *RecordTxtDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordTxt, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordTxtAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordTxtModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordTxtAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordTxtAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordTxtDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordTxt from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordTxtResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordTxtDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordTxtService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordTxtDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordTxtDataSourceModel
...
func (d *RecordTxtDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": RecordTxtType},
	...
	var allResults []*coremodel.RecordTxt
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordTxt, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordTxt records: %s", err))
		return
	}
