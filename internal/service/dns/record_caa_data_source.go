...
var _ datasource.DataSourceWithValidateConfig = &RecordCaaDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordCaaDataSource{}
...
func (d *RecordCaaDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_caa"
}

// RecordCaaDataSourceModel is the filter model for the datasource
type RecordCaaDataSourceModel struct {
...
func (m *RecordCaaDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordCaa, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordCaaAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordCaaModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordCaaAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordCaaAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordCaaDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordCaa from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordCaaResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordCaaDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordCaaService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordCaaDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordCaaDataSourceModel
...
func (d *RecordCaaDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": RecordCaaType},
	...
	var allResults []*coremodel.RecordCaa
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordCaa, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordCaa records: %s", err))
		return
	}
