...
var (
	_ resource.Resource                   = &NsgroupStubmemberResource{}
	_ resource.ResourceWithValidateConfig = &NsgroupStubmemberResource{}
	_ resource.ResourceWithConfigure      = &NsgroupStubmemberResource{}
	_ resource.ResourceWithImportState    = &NsgroupStubmemberResource{}
	_ resource.ResourceWithIdentity       = &NsgroupStubmemberResource{}
)
...
type NsgroupStubmemberResource struct {
	...
	service coresvc.NsgroupStubmemberService
}

func (r *NsgroupStubmemberResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_nsgroup_stubmember"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *NsgroupStubmemberResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *NsgroupStubmemberResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox NsgroupStubmember in the NIOS backend.",
		Attributes:          NsgroupStubmemberResourceSchemaAttributes,
	}
}

func (r *NsgroupStubmemberResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewNsgroupStubmemberService(r.backend, client.NIOS, client.UDDI)
}

func (r *NsgroupStubmemberResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.NsgroupStubmember](r.backend, op)
}

func (r *NsgroupStubmemberResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateNsgroupStubmember(ctx, data, resp)
}
...
func (r *NsgroupStubmemberResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSNsgroupStubmemberModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSNsgroupStubmemberModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSNsgroupStubmemberAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.NsgroupStubmember
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: NsgroupStubmemberReturnFields,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create NsgroupStubmember: %s", err))
		return
	}
...
func (r *NsgroupStubmemberResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read NsgroupStubmember: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSNsgroupStubmemberModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read NsgroupStubmember because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *NsgroupStubmemberResource) ReadByExtAttrs(ctx context.Context, data *NsgroupStubmemberModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.NsgroupStubmember
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search NsgroupStubmember by extattrs: %s", err))
		return true
	}
...
func (r *NsgroupStubmemberResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSNsgroupStubmemberModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSNsgroupStubmemberModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSNsgroupStubmemberModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSNsgroupStubmemberAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update NsgroupStubmember: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSNsgroupStubmemberModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSNsgroupStubmemberAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *NsgroupStubmemberResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete NsgroupStubmember: %s", err))
	}
}
