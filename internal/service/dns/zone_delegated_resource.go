...
var (
	_ resource.Resource                   = &ZoneDelegatedResource{}
	_ resource.ResourceWithValidateConfig = &ZoneDelegatedResource{}
	_ resource.ResourceWithConfigure      = &ZoneDelegatedResource{}
	_ resource.ResourceWithImportState    = &ZoneDelegatedResource{}
	_ resource.ResourceWithIdentity       = &ZoneDelegatedResource{}
)
...
func (r *ZoneDelegatedResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (r *ZoneDelegatedResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *ZoneDelegatedResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox ZoneDelegated in both NIOS and UDDI backends.",
		Attributes:          ZoneDelegatedResourceSchemaAttributes,
	}
}

func (r *ZoneDelegatedResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewZoneDelegatedService(r.backend, client.NIOS, client.UDDI)
}

func (r *ZoneDelegatedResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.ZoneDelegated](r.backend, op)
}
...
func (r *ZoneDelegatedResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateZoneDelegated(ctx, data, resp)
}
...
func (r *ZoneDelegatedResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSZoneDelegatedModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSZoneDelegatedModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSZoneDelegatedAttrTypes, &resp.Diagnostics)
	}
	...
	if r.backend == core.BackendNIOS {
		ApplyZoneDelegatedNIOSUseFlags(ctx, req.Config, obj, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
	}
	...
	var (
		apiResp  *coremodel.ZoneDelegated
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create ZoneDelegated: %s", err))
		return
	}
...
func (r *ZoneDelegatedResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read ZoneDelegated: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSZoneDelegatedModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read ZoneDelegated because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *ZoneDelegatedResource) ReadByExtAttrs(ctx context.Context, data *ZoneDelegatedModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.ZoneDelegated
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search ZoneDelegated by extattrs: %s", err))
		return true
	}
...
func (r *ZoneDelegatedResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSZoneDelegatedModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSZoneDelegatedModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSZoneDelegatedModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSZoneDelegatedAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update ZoneDelegated: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSZoneDelegatedModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSZoneDelegatedAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *ZoneDelegatedResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete ZoneDelegated: %s", err))
	}
}
