...
func (l *RecordAaaaList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_aaaa"
}

func (l *RecordAaaaList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordAaaaService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordAaaaList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordAaaa from the configured backend (NIOS or UDDI).",
...
func (l *RecordAaaaList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_aaaa list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordAaaa
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordAaaa, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordAaaa, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordAaaa records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
