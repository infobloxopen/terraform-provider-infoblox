...
var _ datasource.DataSourceWithValidateConfig = &RecordADataSource{}
var _ datasource.DataSourceWithConfigure = &RecordADataSource{}
...
func (d *RecordADataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_a"
}

// RecordADataSourceModel is the filter model for the datasource
type RecordADataSourceModel struct {
...
func (m *RecordADataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordA, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordAAttrTypes})
		return
	}
	...
	for _, rec := range from {
		...
		obj, d := types.ObjectValueFrom(ctx, RecordAAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, obj)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordAAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordADataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordA across NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordAResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordADataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordAService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordADataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordADataSourceModel
...
func (d *RecordADataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": RecordAType},
	...
	var records []*coremodel.RecordA
	var err error

	switch d.backend {
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordA, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.RecordA, error) {
			opts.Offset = offset
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordA records: %s", err))
		return
	}
