...
func (m *RecordADataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordA, diags *diag.Diagnostics) {
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, RecordAAttrTypes, model)
		diags.Append(d...)
...
func (d *RecordADataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordA from both the NIOS and UDDI backends.",
...
func (d *RecordADataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.RecordA
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordA, string, error) {
			opts.PageID = pageID
