...
var (
	_ list.ListResource                   = &RecordSvcbList{}
	_ list.ListResourceWithConfigure      = &RecordSvcbList{}
	_ list.ListResourceWithValidateConfig = &RecordSvcbList{}
)

func NewRecordSvcbList() list.ListResource {
	return &RecordSvcbList{}
}

type RecordSvcbList struct {
	backend core.BackendType
	service coresvc.RecordSvcbService
}

type RecordSvcbListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *RecordSvcbList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_svcb"
}

func (l *RecordSvcbList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordSvcbService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordSvcbList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordSvcb from the UDDI backend.",
...
func (l *RecordSvcbList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data RecordSvcbListModel
...
func (l *RecordSvcbList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RecordSvcbListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_svcb list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordSvcb
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordSvcb, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordSvcb, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordSvcb records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
