...
var _ datasource.DataSourceWithValidateConfig = &RecordAliasDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordAliasDataSource{}

func NewRecordAliasDataSource() datasource.DataSource {
	return &RecordAliasDataSource{}
}

type RecordAliasDataSource struct {
	backend core.BackendType
	service coresvc.RecordAliasService
}

func (d *RecordAliasDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_alias"
}

// RecordAliasDataSourceModel is the filter model for the datasource
...
func (m *RecordAliasDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordAlias, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordAliasAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordAliasModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordAliasAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordAliasAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}
...
func (d *RecordAliasDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordAliasService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordAliasDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordAliasDataSourceModel
...
func (d *RecordAliasDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.RecordAlias
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordAlias, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordAlias records: %s", err))
		return
	}
