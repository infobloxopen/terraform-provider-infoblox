...
var _ datasource.DataSourceWithValidateConfig = &RecordAaaaDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordAaaaDataSource{}
...
func (d *RecordAaaaDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_aaaa"
}

// RecordAaaaDataSourceModel is the filter model for the datasource
type RecordAaaaDataSourceModel struct {
...
func (m *RecordAaaaDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordAaaa, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordAaaaAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordAaaaModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordAaaaAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordAaaaAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordAaaaDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordAaaa from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordAaaaResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordAaaaDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordAaaaService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordAaaaDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordAaaaDataSourceModel
...
func (d *RecordAaaaDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": RecordAaaaType},
	...
	var allResults []*coremodel.RecordAaaa
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordAaaa, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordAaaa records: %s", err))
		return
	}
