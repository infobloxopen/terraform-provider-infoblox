...
func (l *ZoneAuthList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_zone_auth"
}

func (l *ZoneAuthList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewZoneAuthService(l.backend, client.NIOS, client.UDDI)
}
...
func (l *ZoneAuthList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_zone_auth list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.ZoneAuth
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.ZoneAuth, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.ZoneAuth, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list ZoneAuth records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
