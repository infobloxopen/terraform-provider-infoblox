...
func (l *RecordAList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordAService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordAList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordA from the configured backend (NIOS or UDDI).",
...
func (l *RecordAList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordA, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordA, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordA records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
