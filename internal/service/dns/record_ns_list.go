...
func (l *RecordNsList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordNsService(l.backend, client.NIOS, client.UDDI)
}
...
func (l *RecordNsList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	var records []*coremodel.RecordNs
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordNs, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordNs, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordNs records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
