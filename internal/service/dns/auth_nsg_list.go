...
var (
	_ list.ListResource                   = &AuthNsgList{}
	_ list.ListResourceWithConfigure      = &AuthNsgList{}
	_ list.ListResourceWithValidateConfig = &AuthNsgList{}
)

func NewAuthNsgList() list.ListResource {
	return &AuthNsgList{}
}

type AuthNsgList struct {
	backend core.BackendType
	service coresvc.AuthNsgService
}

type AuthNsgListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *AuthNsgList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_auth_nsg"
}

func (l *AuthNsgList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewAuthNsgService(l.backend, client.NIOS, client.UDDI)
}

func (l *AuthNsgList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox AuthNsg from the UDDI backend.",
...
func (l *AuthNsgList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data AuthNsgListModel
...
func (l *AuthNsgList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data AuthNsgListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_auth_nsg list: req.Limit=%d backend=%s includeResource=%t",
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.AuthNsg, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.AuthNsg, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list AuthNsg records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
