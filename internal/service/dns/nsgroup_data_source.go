...
var _ datasource.DataSourceWithValidateConfig = &NsgroupDataSource{}
var _ datasource.DataSourceWithConfigure = &NsgroupDataSource{}
...
// NsgroupDataSourceModel is the filter model for the datasource
...
func (m *NsgroupDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.Nsgroup, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: NsgroupAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, NsgroupAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: NsgroupAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}
...
func (d *NsgroupDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox Nsgroup across NIOS and UDDI backends.",
...
func (d *NsgroupDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewNsgroupService(d.backend, client.NIOS, client.UDDI)
}

func (d *NsgroupDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data NsgroupDataSourceModel
...
func (d *NsgroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.Nsgroup
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.Nsgroup, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list Nsgroup records: %s", err))
		return
	}
