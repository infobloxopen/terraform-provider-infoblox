...
var (
	_ resource.Resource                   = &SharedrecordgroupResource{}
	_ resource.ResourceWithValidateConfig = &SharedrecordgroupResource{}
	_ resource.ResourceWithConfigure      = &SharedrecordgroupResource{}
	_ resource.ResourceWithImportState    = &SharedrecordgroupResource{}
	_ resource.ResourceWithIdentity       = &SharedrecordgroupResource{}
)
...
type SharedrecordgroupResource struct {
	...
	service coresvc.SharedrecordgroupService
}
...
func (r *SharedrecordgroupResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *SharedrecordgroupResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox Sharedrecordgroup in the NIOS backend.",
		Attributes:          SharedrecordgroupResourceSchemaAttributes,
	}
}

func (r *SharedrecordgroupResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewSharedrecordgroupService(r.backend, client.NIOS, client.UDDI)
}

func (r *SharedrecordgroupResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.Sharedrecordgroup](r.backend, op)
}
...
func (r *SharedrecordgroupResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateSharedrecordgroup(ctx, data, resp)
}
...
func (r *SharedrecordgroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSSharedrecordgroupModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSSharedrecordgroupModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSSharedrecordgroupAttrTypes, &resp.Diagnostics)
	}
	...
	if r.backend == core.BackendNIOS {
		ApplySharedrecordgroupNIOSUseFlags(ctx, req.Config, obj, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
	}
	...
	var (
		apiResp  *coremodel.Sharedrecordgroup
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: SharedrecordgroupReturnFields,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create Sharedrecordgroup: %s", err))
		return
	}
...
func (r *SharedrecordgroupResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Sharedrecordgroup: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSSharedrecordgroupModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read Sharedrecordgroup because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *SharedrecordgroupResource) ReadByExtAttrs(ctx context.Context, data *SharedrecordgroupModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.Sharedrecordgroup
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search Sharedrecordgroup by extattrs: %s", err))
		return true
	}
...
func (r *SharedrecordgroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSSharedrecordgroupModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSSharedrecordgroupModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSSharedrecordgroupModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSSharedrecordgroupAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update Sharedrecordgroup: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSSharedrecordgroupModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSSharedrecordgroupAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *SharedrecordgroupResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete Sharedrecordgroup: %s", err))
	}
}
