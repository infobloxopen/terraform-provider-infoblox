...
var _ datasource.DataSourceWithValidateConfig = &RecordCnameDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordCnameDataSource{}
...
func (d *RecordCnameDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_cname"
}

// RecordCnameDataSourceModel is the filter model for the datasource
type RecordCnameDataSourceModel struct {
...
func (m *RecordCnameDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordCname, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordCnameAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordCnameModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordCnameAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordCnameAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordCnameDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordCname from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordCnameResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordCnameDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordCnameService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordCnameDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordCnameDataSourceModel
...
func (d *RecordCnameDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": RecordCnameType},
	...
	var allResults []*coremodel.RecordCname
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordCname, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordCname records: %s", err))
		return
	}
