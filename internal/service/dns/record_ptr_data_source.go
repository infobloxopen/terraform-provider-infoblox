...
var _ datasource.DataSourceWithValidateConfig = &RecordPtrDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordPtrDataSource{}
...
type RecordPtrDataSource struct {
	...
	service coresvc.RecordPtrService
}

func (d *RecordPtrDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_ptr"
}

// RecordPtrDataSourceModel is the filter model for the datasource
type RecordPtrDataSourceModel struct {
...
func (m *RecordPtrDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordPtr, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordPtrAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordPtrModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordPtrAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordPtrAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordPtrDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordPtr from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordPtrResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordPtrDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordPtrService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordPtrDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordPtrDataSourceModel
...
func (d *RecordPtrDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": RecordPtrType},
	...
	var allResults []*coremodel.RecordPtr
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordPtr, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.RecordPtr, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordPtr records: %s", err))
		return
	}
