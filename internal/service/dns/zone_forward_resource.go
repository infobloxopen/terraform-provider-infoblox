...
var (
	_ resource.Resource                   = &ZoneForwardResource{}
	_ resource.ResourceWithValidateConfig = &ZoneForwardResource{}
	_ resource.ResourceWithConfigure      = &ZoneForwardResource{}
	_ resource.ResourceWithImportState    = &ZoneForwardResource{}
	_ resource.ResourceWithIdentity       = &ZoneForwardResource{}
)
...
type ZoneForwardResource struct {
	...
	service coresvc.ZoneForwardService
}

func (r *ZoneForwardResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_zone_forward"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *ZoneForwardResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *ZoneForwardResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox ZoneForward in both NIOS and UDDI backends.",
		Attributes:          ZoneForwardResourceSchemaAttributes,
	}
}

func (r *ZoneForwardResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewZoneForwardService(r.backend, client.NIOS, client.UDDI)
}

func (r *ZoneForwardResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.ZoneForward](r.backend, op)
}
...
func (r *ZoneForwardResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateZoneForward(ctx, data, resp)
}
...
func (r *ZoneForwardResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSZoneForwardModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSZoneForwardModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSZoneForwardAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create ZoneForward: %s", err))
		return
	}
...
func (r *ZoneForwardResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read ZoneForward: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSZoneForwardModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read ZoneForward because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *ZoneForwardResource) ReadByExtAttrs(ctx context.Context, data *ZoneForwardModel, resp *resource.ReadResponse) bool {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search ZoneForward by extattrs: %s", err))
		return true
	}
...
func (r *ZoneForwardResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSZoneForwardModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSZoneForwardModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSZoneForwardModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSZoneForwardAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update ZoneForward: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSZoneForwardModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSZoneForwardAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *ZoneForwardResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete ZoneForward: %s", err))
	}
}
