...
var _ datasource.DataSourceWithValidateConfig = &RecordDnameDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordDnameDataSource{}
...
func (d *RecordDnameDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_dname"
}

// RecordDnameDataSourceModel is the filter model for the datasource
type RecordDnameDataSourceModel struct {
...
func (m *RecordDnameDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordDname, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordDnameAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordDnameModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordDnameAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordDnameAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordDnameDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordDname from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordDnameResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordDnameDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordDnameService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordDnameDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordDnameDataSourceModel
...
func (d *RecordDnameDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": RecordDnameType},
	...
	var allResults []*coremodel.RecordDname
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordDname, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordDname records: %s", err))
		return
	}
