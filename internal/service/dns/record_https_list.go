...
var (
	_ list.ListResource                   = &RecordHttpsList{}
	_ list.ListResourceWithConfigure      = &RecordHttpsList{}
	_ list.ListResourceWithValidateConfig = &RecordHttpsList{}
)

func NewRecordHttpsList() list.ListResource {
	return &RecordHttpsList{}
}

type RecordHttpsList struct {
	backend core.BackendType
	service coresvc.RecordHttpsService
}

type RecordHttpsListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *RecordHttpsList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_https"
}

func (l *RecordHttpsList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordHttpsService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordHttpsList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordHttps from the UDDI backend.",
...
func (l *RecordHttpsList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data RecordHttpsListModel
...
func (l *RecordHttpsList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RecordHttpsListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_https list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordHttps
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordHttps, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordHttps, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordHttps records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
