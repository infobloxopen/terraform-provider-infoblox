...
func (l *RecordCnameList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_cname"
}

func (l *RecordCnameList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordCnameService(l.backend, client.NIOS, client.UDDI)
}
...
func (l *RecordCnameList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_cname list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordCname
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordCname, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordCname, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordCname records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
