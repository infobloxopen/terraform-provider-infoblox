...
var (
	_ resource.Resource                   = &NsgroupForwardstubserverResource{}
	_ resource.ResourceWithValidateConfig = &NsgroupForwardstubserverResource{}
	_ resource.ResourceWithConfigure      = &NsgroupForwardstubserverResource{}
	_ resource.ResourceWithImportState    = &NsgroupForwardstubserverResource{}
	_ resource.ResourceWithIdentity       = &NsgroupForwardstubserverResource{}
)
...
func (r *NsgroupForwardstubserverResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (r *NsgroupForwardstubserverResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *NsgroupForwardstubserverResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox NsgroupForwardstubserver in the NIOS backend.",
		Attributes:          NsgroupForwardstubserverResourceSchemaAttributes,
	}
}

func (r *NsgroupForwardstubserverResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewNsgroupForwardstubserverService(r.backend, client.NIOS, client.UDDI)
}

func (r *NsgroupForwardstubserverResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.NsgroupForwardstubserver](r.backend, op)
}

func (r *NsgroupForwardstubserverResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateNsgroupForwardstubserver(ctx, data, resp)
}
...
func (r *NsgroupForwardstubserverResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSNsgroupForwardstubserverModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSNsgroupForwardstubserverModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSNsgroupForwardstubserverAttrTypes, &resp.Diagnostics)
	}
	...
	var (
		apiResp  *coremodel.NsgroupForwardstubserver
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create NsgroupForwardstubserver: %s", err))
		return
	}
...
func (r *NsgroupForwardstubserverResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read NsgroupForwardstubserver: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSNsgroupForwardstubserverModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read NsgroupForwardstubserver because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *NsgroupForwardstubserverResource) ReadByExtAttrs(ctx context.Context, data *NsgroupForwardstubserverModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.NsgroupForwardstubserver
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search NsgroupForwardstubserver by extattrs: %s", err))
		return true
	}
...
func (r *NsgroupForwardstubserverResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSNsgroupForwardstubserverModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSNsgroupForwardstubserverModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSNsgroupForwardstubserverModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSNsgroupForwardstubserverAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update NsgroupForwardstubserver: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSNsgroupForwardstubserverModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSNsgroupForwardstubserverAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *NsgroupForwardstubserverResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete NsgroupForwardstubserver: %s", err))
	}
}
