...
func NewExtensibleattributedefList() list.ListResource {
	return &ExtensibleattributedefList{}
}
...
func (l *ExtensibleattributedefList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewExtensibleattributedefService(l.backend, client.NIOS, client.UDDI)
}

func (l *ExtensibleattributedefList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Extensibleattributedef from the NIOS backend.",
...
func (l *ExtensibleattributedefList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	var records []*coremodel.Extensibleattributedef
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Extensibleattributedef, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Extensibleattributedef, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Extensibleattributedef records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
