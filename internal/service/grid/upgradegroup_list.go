...
var (
	_ list.ListResource                   = &UpgradegroupList{}
	_ list.ListResourceWithConfigure      = &UpgradegroupList{}
	_ list.ListResourceWithValidateConfig = &UpgradegroupList{}
)

func NewUpgradegroupList() list.ListResource {
	return &UpgradegroupList{}
}

type UpgradegroupList struct {
	backend core.BackendType
	service coresvc.UpgradegroupService
}

type UpgradegroupListModel struct {
	Filters types.Map `tfsdk:"filters"`
}

func (l *UpgradegroupList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_upgradegroup"
}

func (l *UpgradegroupList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewUpgradegroupService(l.backend, client.NIOS, client.UDDI)
}

func (l *UpgradegroupList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Upgradegroup from the NIOS backend.",
...
func (l *UpgradegroupList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data UpgradegroupListModel
...
func (l *UpgradegroupList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data UpgradegroupListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_upgradegroup list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.Upgradegroup
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Upgradegroup, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Upgradegroup, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Upgradegroup records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
