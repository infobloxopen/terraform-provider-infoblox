...
var (
	_ list.ListResource                   = &SuperhostList{}
	_ list.ListResourceWithConfigure      = &SuperhostList{}
	_ list.ListResourceWithValidateConfig = &SuperhostList{}
)
...
func (l *SuperhostList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_superhost"
}

func (l *SuperhostList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewSuperhostService(l.backend, client.NIOS, client.UDDI)
}

func (l *SuperhostList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Superhost from the NIOS backend.",
...
func (l *SuperhostList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
...
func (l *SuperhostList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_superhost list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.Superhost
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Superhost, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Superhost, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Superhost records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
