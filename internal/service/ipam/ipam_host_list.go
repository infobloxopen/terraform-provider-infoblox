...
var (
	_ list.ListResource                   = &IpamHostList{}
	_ list.ListResourceWithConfigure      = &IpamHostList{}
	_ list.ListResourceWithValidateConfig = &IpamHostList{}
)

func NewIpamHostList() list.ListResource {
	return &IpamHostList{}
}
...
type IpamHostListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *IpamHostList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_ipam_host"
}

func (l *IpamHostList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewIpamHostService(l.backend, client.NIOS, client.UDDI)
}

func (l *IpamHostList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox IpamHost from the UDDI backend.",
...
func (l *IpamHostList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data IpamHostListModel
...
func (l *IpamHostList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data IpamHostListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_ipam_host list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.IpamHost
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.IpamHost, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.IpamHost, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list IpamHost records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
