...
var (
	_ resource.Resource                   = &Ipv6networkcontainerResource{}
	_ resource.ResourceWithValidateConfig = &Ipv6networkcontainerResource{}
	_ resource.ResourceWithConfigure      = &Ipv6networkcontainerResource{}
	_ resource.ResourceWithImportState    = &Ipv6networkcontainerResource{}
	_ resource.ResourceWithIdentity       = &Ipv6networkcontainerResource{}
)
...
func (r *Ipv6networkcontainerResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (r *Ipv6networkcontainerResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewIpv6networkcontainerService(r.backend, client.NIOS, client.UDDI)
}

func (r *Ipv6networkcontainerResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.Ipv6networkcontainer](r.backend, op)
}
...
func (r *Ipv6networkcontainerResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSIpv6networkcontainerModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSIpv6networkcontainerModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSIpv6networkcontainerAttrTypes, &resp.Diagnostics)
	}
	...
	if r.backend == core.BackendNIOS {
		ApplyIpv6networkcontainerNIOSUseFlags(ctx, req.Config, obj, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
	}
	...
	var (
		apiResp  *coremodel.Ipv6networkcontainer
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create Ipv6networkcontainer: %s", err))
		return
	}
...
func (r *Ipv6networkcontainerResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Ipv6networkcontainer: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSIpv6networkcontainerModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read Ipv6networkcontainer because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *Ipv6networkcontainerResource) ReadByExtAttrs(ctx context.Context, data *Ipv6networkcontainerModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.Ipv6networkcontainer
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search Ipv6networkcontainer by extattrs: %s", err))
		return true
	}
...
func (r *Ipv6networkcontainerResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSIpv6networkcontainerModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSIpv6networkcontainerModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSIpv6networkcontainerModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSIpv6networkcontainerAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update Ipv6networkcontainer: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSIpv6networkcontainerModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSIpv6networkcontainerAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *Ipv6networkcontainerResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete Ipv6networkcontainer: %s", err))
	}
}
