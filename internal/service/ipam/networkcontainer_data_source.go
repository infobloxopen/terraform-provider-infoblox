...
var _ datasource.DataSourceWithValidateConfig = &NetworkcontainerDataSource{}
var _ datasource.DataSourceWithConfigure = &NetworkcontainerDataSource{}
...
type NetworkcontainerDataSourceModel struct {
...
func (m *NetworkcontainerDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.Networkcontainer, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: NetworkcontainerAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &NetworkcontainerModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, NetworkcontainerAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: NetworkcontainerAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *NetworkcontainerDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox Networkcontainer from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(NetworkcontainerResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *NetworkcontainerDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewNetworkcontainerService(d.backend, client.NIOS, client.UDDI)
}

func (d *NetworkcontainerDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data NetworkcontainerDataSourceModel
...
func (d *NetworkcontainerDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.Networkcontainer
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.Networkcontainer, string, error) {
			opts.PageID = pageID
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list Networkcontainer records: %s", err))
		return
	}
