...
var (
	_ resource.Resource                   = &NetworkcontainerResource{}
	_ resource.ResourceWithValidateConfig = &NetworkcontainerResource{}
	_ resource.ResourceWithConfigure      = &NetworkcontainerResource{}
	_ resource.ResourceWithImportState    = &NetworkcontainerResource{}
	_ resource.ResourceWithIdentity       = &NetworkcontainerResource{}
)
...
type NetworkcontainerResource struct {
	...
	service coresvc.NetworkcontainerService
}
...
func (r *NetworkcontainerResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *NetworkcontainerResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox Networkcontainer in both NIOS and UDDI backends.",
		Attributes:          NetworkcontainerResourceSchemaAttributes,
	}
}

func (r *NetworkcontainerResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewNetworkcontainerService(r.backend, client.NIOS, client.UDDI)
}

func (r *NetworkcontainerResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.Networkcontainer](r.backend, op)
}
...
func (r *NetworkcontainerResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateNetworkcontainer(ctx, data, resp)
}
...
func (r *NetworkcontainerResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	unlock := LockNetworkcontainerAllocation(ctx, data.UDDI, &resp.Diagnostics)
	defer unlock()
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSNetworkcontainerModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSNetworkcontainerModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSNetworkcontainerAttrTypes, &resp.Diagnostics)
	}
	...
	if r.backend == core.BackendNIOS {
		ApplyNetworkcontainerNIOSUseFlags(ctx, req.Config, obj, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
	}
	...
	var (
		apiResp  *coremodel.Networkcontainer
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			...
			Inherit:      NetworkcontainerInheritanceType,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create Networkcontainer: %s", err))
		return
	}
...
func (r *NetworkcontainerResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Networkcontainer: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSNetworkcontainerModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read Networkcontainer because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *NetworkcontainerResource) ReadByExtAttrs(ctx context.Context, data *NetworkcontainerModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.Networkcontainer
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search Networkcontainer by extattrs: %s", err))
		return true
	}
...
func (r *NetworkcontainerResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSNetworkcontainerModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSNetworkcontainerModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSNetworkcontainerModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSNetworkcontainerAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update Networkcontainer: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSNetworkcontainerModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSNetworkcontainerAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *NetworkcontainerResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete Networkcontainer: %s", err))
	}
}
