...
func (l *NetworkcontainerList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewNetworkcontainerService(l.backend, client.NIOS, client.UDDI)
}

func (l *NetworkcontainerList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Networkcontainer from the configured backend (NIOS or UDDI).",
...
func (l *NetworkcontainerList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	var records []*coremodel.Networkcontainer
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Networkcontainer, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Networkcontainer, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Networkcontainer records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
