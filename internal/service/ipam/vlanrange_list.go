...
var (
	_ list.ListResource                   = &VlanrangeList{}
	_ list.ListResourceWithConfigure      = &VlanrangeList{}
	_ list.ListResourceWithValidateConfig = &VlanrangeList{}
)
...
func (l *VlanrangeList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_vlan_range"
}

func (l *VlanrangeList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewVlanrangeService(l.backend, client.NIOS, client.UDDI)
}

func (l *VlanrangeList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Vlanrange from the NIOS backend.",
...
func (l *VlanrangeList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
...
func (l *VlanrangeList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_vlan_range list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.Vlanrange
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Vlanrange, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Vlanrange, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Vlanrange records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
