...
var (
	_ list.ListResource                   = &DhcpOptionspaceList{}
	_ list.ListResourceWithConfigure      = &DhcpOptionspaceList{}
	_ list.ListResourceWithValidateConfig = &DhcpOptionspaceList{}
)

func NewDhcpOptionspaceList() list.ListResource {
	return &DhcpOptionspaceList{}
}

type DhcpOptionspaceList struct {
	backend core.BackendType
	service coresvc.DhcpOptionspaceService
}

type DhcpOptionspaceListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *DhcpOptionspaceList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_dhcp_optionspace"
}

func (l *DhcpOptionspaceList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDhcpOptionspaceService(l.backend, client.NIOS, client.UDDI)
}

func (l *DhcpOptionspaceList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox DhcpOptionspace from both the NIOS and UDDI backends.",
...
func (l *DhcpOptionspaceList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DhcpOptionspaceListModel
...
func (l *DhcpOptionspaceList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data DhcpOptionspaceListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dhcp_optionspace list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.DhcpOptionspace
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DhcpOptionspace, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DhcpOptionspace, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DhcpOptionspace records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
