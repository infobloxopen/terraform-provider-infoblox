...
var _ datasource.DataSourceWithValidateConfig = &Ipv6DhcpOptiondefinitionDataSource{}
var _ datasource.DataSourceWithConfigure = &Ipv6DhcpOptiondefinitionDataSource{}
...
func (d *Ipv6DhcpOptiondefinitionDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_ipv6_dhcp_optiondefinition"
}

// Ipv6DhcpOptiondefinitionDataSourceModel is the filter model for the datasource
type Ipv6DhcpOptiondefinitionDataSourceModel struct {
...
func (m *Ipv6DhcpOptiondefinitionDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.Ipv6DhcpOptiondefinition, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: Ipv6DhcpOptiondefinitionAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &Ipv6DhcpOptiondefinitionModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, Ipv6DhcpOptiondefinitionAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: Ipv6DhcpOptiondefinitionAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *Ipv6DhcpOptiondefinitionDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox Ipv6DhcpOptiondefinition from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(Ipv6DhcpOptiondefinitionResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *Ipv6DhcpOptiondefinitionDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewIpv6DhcpOptiondefinitionService(d.backend, client.NIOS, client.UDDI)
}

func (d *Ipv6DhcpOptiondefinitionDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data Ipv6DhcpOptiondefinitionDataSourceModel
...
func (d *Ipv6DhcpOptiondefinitionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	opts := &core.ListOptions{
		...
		InternalFilters: map[string]string{"type": Ipv6DhcpOptiondefinitionType},
	...
	var allResults []*coremodel.Ipv6DhcpOptiondefinition
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.Ipv6DhcpOptiondefinition, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.Ipv6DhcpOptiondefinition, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list Ipv6DhcpOptiondefinition records: %s", err))
		return
	}
