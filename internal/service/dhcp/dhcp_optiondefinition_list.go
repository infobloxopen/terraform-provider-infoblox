...
var (
	_ list.ListResource                   = &DhcpOptiondefinitionList{}
	_ list.ListResourceWithConfigure      = &DhcpOptiondefinitionList{}
	_ list.ListResourceWithValidateConfig = &DhcpOptiondefinitionList{}
)

func NewDhcpOptiondefinitionList() list.ListResource {
	return &DhcpOptiondefinitionList{}
}

type DhcpOptiondefinitionList struct {
...
func (l *DhcpOptiondefinitionList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewDhcpOptiondefinitionService(l.backend, client.NIOS, client.UDDI)
}

func (l *DhcpOptiondefinitionList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox DhcpOptiondefinition from both the NIOS and UDDI backends.",
...
func (l *DhcpOptiondefinitionList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data DhcpOptiondefinitionListModel
...
func (l *DhcpOptiondefinitionList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_dhcp_optiondefinition list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.DhcpOptiondefinition
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.DhcpOptiondefinition, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.DhcpOptiondefinition, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list DhcpOptiondefinition records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
