...
var (
	_ list.ListResource                   = &HardwareFilterList{}
	_ list.ListResourceWithConfigure      = &HardwareFilterList{}
	_ list.ListResourceWithValidateConfig = &HardwareFilterList{}
)

func NewHardwareFilterList() list.ListResource {
	return &HardwareFilterList{}
}

type HardwareFilterList struct {
	backend core.BackendType
	service coresvc.HardwareFilterService
}

type HardwareFilterListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *HardwareFilterList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_hardware_filter"
}

func (l *HardwareFilterList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewHardwareFilterService(l.backend, client.NIOS, client.UDDI)
}

func (l *HardwareFilterList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox HardwareFilter from the UDDI backend.",
...
func (l *HardwareFilterList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data HardwareFilterListModel
...
func (l *HardwareFilterList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data HardwareFilterListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_hardware_filter list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.HardwareFilter
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.HardwareFilter, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.HardwareFilter, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list HardwareFilter records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
