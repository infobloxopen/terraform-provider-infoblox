...
var (
	_ list.ListResource                   = &OptionGroupList{}
	_ list.ListResourceWithConfigure      = &OptionGroupList{}
	_ list.ListResourceWithValidateConfig = &OptionGroupList{}
)

func NewOptionGroupList() list.ListResource {
...
type OptionGroupListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *OptionGroupList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_option_group"
}

func (l *OptionGroupList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewOptionGroupService(l.backend, client.NIOS, client.UDDI)
}

func (l *OptionGroupList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox OptionGroup from the UDDI backend.",
...
func (l *OptionGroupList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data OptionGroupListModel
...
func (l *OptionGroupList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data OptionGroupListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_option_group list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.OptionGroup
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.OptionGroup, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.OptionGroup, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list OptionGroup records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
