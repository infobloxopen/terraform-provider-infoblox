...
var _ datasource.DataSourceWithValidateConfig = &Ipv6fixedaddresstemplateDataSource{}
var _ datasource.DataSourceWithConfigure = &Ipv6fixedaddresstemplateDataSource{}
...
type Ipv6fixedaddresstemplateDataSourceModel struct {
...
func (m *Ipv6fixedaddresstemplateDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.Ipv6fixedaddresstemplate, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: Ipv6fixedaddresstemplateAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &Ipv6fixedaddresstemplateModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, Ipv6fixedaddresstemplateAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: Ipv6fixedaddresstemplateAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *Ipv6fixedaddresstemplateDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox Ipv6fixedaddresstemplate from the NIOS backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(Ipv6fixedaddresstemplateResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *Ipv6fixedaddresstemplateDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewIpv6fixedaddresstemplateService(d.backend, client.NIOS, client.UDDI)
}

func (d *Ipv6fixedaddresstemplateDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data Ipv6fixedaddresstemplateDataSourceModel
...
func (d *Ipv6fixedaddresstemplateDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.Ipv6fixedaddresstemplate
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.Ipv6fixedaddresstemplate, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.Ipv6fixedaddresstemplate, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list Ipv6fixedaddresstemplate records: %s", err))
		return
	}
