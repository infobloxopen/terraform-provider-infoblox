...
func (r *Ipv6fixedaddresstemplateResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSIpv6fixedaddresstemplateModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSIpv6fixedaddresstemplateModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSIpv6fixedaddresstemplateAttrTypes, &resp.Diagnostics)
	}
...
func (r *Ipv6fixedaddresstemplateResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSIpv6fixedaddresstemplateModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read Ipv6fixedaddresstemplate because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *Ipv6fixedaddresstemplateResource) ReadByExtAttrs(ctx context.Context, data *Ipv6fixedaddresstemplateModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.Ipv6fixedaddresstemplate
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search Ipv6fixedaddresstemplate by extattrs: %s", err))
		return true
	}
