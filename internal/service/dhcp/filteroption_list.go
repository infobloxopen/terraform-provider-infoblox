...
var (
	_ list.ListResource                   = &FilteroptionList{}
	_ list.ListResourceWithConfigure      = &FilteroptionList{}
	_ list.ListResourceWithValidateConfig = &FilteroptionList{}
)

func NewFilteroptionList() list.ListResource {
...
type FilteroptionListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
	TagFilters     types.Map `tfsdk:"tag_filters"`
}

func (l *FilteroptionList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_filteroption"
}

func (l *FilteroptionList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewFilteroptionService(l.backend, client.NIOS, client.UDDI)
}

func (l *FilteroptionList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Filteroption from both the NIOS and UDDI backends.",
...
func (l *FilteroptionList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data FilteroptionListModel
...
func (l *FilteroptionList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data FilteroptionListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_filteroption list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.Filteroption
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Filteroption, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Filteroption, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Filteroption records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
