...
var (
	_ list.ListResource                   = &SharednetworkList{}
	_ list.ListResourceWithConfigure      = &SharednetworkList{}
	_ list.ListResourceWithValidateConfig = &SharednetworkList{}
)

func NewSharednetworkList() list.ListResource {
	return &SharednetworkList{}
}

type SharednetworkList struct {
	backend core.BackendType
	service coresvc.SharednetworkService
}

type SharednetworkListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
}

func (l *SharednetworkList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_sharednetwork"
}

func (l *SharednetworkList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewSharednetworkService(l.backend, client.NIOS, client.UDDI)
}

func (l *SharednetworkList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Sharednetwork from the NIOS backend.",
...
func (l *SharednetworkList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data SharednetworkListModel
...
func (l *SharednetworkList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data SharednetworkListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_sharednetwork list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.Sharednetwork
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Sharednetwork, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Sharednetwork, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Sharednetwork records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
