...
var (
	_ list.ListResource                   = &RangeList{}
	_ list.ListResourceWithConfigure      = &RangeList{}
	_ list.ListResourceWithValidateConfig = &RangeList{}
)

func NewRangeList() list.ListResource {
	return &RangeList{}
}
...
type RangeListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
	TagFilters     types.Map `tfsdk:"tag_filters"`
}

func (l *RangeList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_range"
}

func (l *RangeList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRangeService(l.backend, client.NIOS, client.UDDI)
}

func (l *RangeList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Range from both the NIOS and UDDI backends.",
...
func (l *RangeList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data RangeListModel
...
func (l *RangeList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RangeListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_range list: req.Limit=%d backend=%s includeResource=%t",
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Range, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Range, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Range records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
