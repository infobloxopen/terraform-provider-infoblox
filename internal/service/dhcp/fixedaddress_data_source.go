...
var _ datasource.DataSourceWithValidateConfig = &FixedaddressDataSource{}
var _ datasource.DataSourceWithConfigure = &FixedaddressDataSource{}
...
func (d *FixedaddressDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_fixed_address"
}

// FixedaddressDataSourceModel is the filter model for the datasource
type FixedaddressDataSourceModel struct {
...
func (m *FixedaddressDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.Fixedaddress, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: FixedaddressAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &FixedaddressModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, FixedaddressAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: FixedaddressAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *FixedaddressDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox Fixedaddress from both the NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(FixedaddressResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *FixedaddressDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewFixedaddressService(d.backend, client.NIOS, client.UDDI)
}

func (d *FixedaddressDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data FixedaddressDataSourceModel
...
func (d *FixedaddressDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.Fixedaddress
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.Fixedaddress, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.Fixedaddress, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list Fixedaddress records: %s", err))
		return
	}
