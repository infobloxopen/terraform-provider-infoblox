...
var _ datasource.DataSourceWithValidateConfig = &SharednetworkDataSource{}
var _ datasource.DataSourceWithConfigure = &SharednetworkDataSource{}
...
func (d *SharednetworkDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_sharednetwork"
}

// SharednetworkDataSourceModel is the filter model for the datasource
type SharednetworkDataSourceModel struct {
...
func (m *SharednetworkDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.Sharednetwork, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: SharednetworkAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, SharednetworkAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: SharednetworkAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *SharednetworkDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox Sharednetwork from the NIOS backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(SharednetworkResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *SharednetworkDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewSharednetworkService(d.backend, client.NIOS, client.UDDI)
}

func (d *SharednetworkDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data SharednetworkDataSourceModel
...
func (d *SharednetworkDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.Sharednetwork
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.Sharednetwork, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.Sharednetwork, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list Sharednetwork records: %s", err))
		return
	}
