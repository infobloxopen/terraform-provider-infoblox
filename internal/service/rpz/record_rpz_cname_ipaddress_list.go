...
var (
	_ list.ListResource                   = &RecordRpzCnameIpaddressList{}
	_ list.ListResourceWithConfigure      = &RecordRpzCnameIpaddressList{}
	_ list.ListResourceWithValidateConfig = &RecordRpzCnameIpaddressList{}
)

func NewRecordRpzCnameIpaddressList() list.ListResource {
	return &RecordRpzCnameIpaddressList{}
}

type RecordRpzCnameIpaddressList struct {
	backend core.BackendType
	service coresvc.RecordRpzCnameIpaddressService
}

type RecordRpzCnameIpaddressListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
}

func (l *RecordRpzCnameIpaddressList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_rpz_cname_ipaddress"
}

func (l *RecordRpzCnameIpaddressList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordRpzCnameIpaddressService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordRpzCnameIpaddressList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordRpzCnameIpaddress from the NIOS backend.",
...
func (l *RecordRpzCnameIpaddressList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data RecordRpzCnameIpaddressListModel
...
func (l *RecordRpzCnameIpaddressList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RecordRpzCnameIpaddressListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_rpz_cname_ipaddress list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordRpzCnameIpaddress
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordRpzCnameIpaddress, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordRpzCnameIpaddress, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordRpzCnameIpaddress records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
