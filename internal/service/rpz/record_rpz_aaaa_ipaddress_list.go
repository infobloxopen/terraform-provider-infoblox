...
var (
	_ list.ListResource                   = &RecordRpzAaaaIpaddressList{}
	_ list.ListResourceWithConfigure      = &RecordRpzAaaaIpaddressList{}
	_ list.ListResourceWithValidateConfig = &RecordRpzAaaaIpaddressList{}
)

func NewRecordRpzAaaaIpaddressList() list.ListResource {
	return &RecordRpzAaaaIpaddressList{}
}

type RecordRpzAaaaIpaddressList struct {
	backend core.BackendType
	service coresvc.RecordRpzAaaaIpaddressService
}

type RecordRpzAaaaIpaddressListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
}

func (l *RecordRpzAaaaIpaddressList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_rpz_aaaa_ipaddress"
}

func (l *RecordRpzAaaaIpaddressList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordRpzAaaaIpaddressService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordRpzAaaaIpaddressList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordRpzAaaaIpaddress from the NIOS backend.",
...
func (l *RecordRpzAaaaIpaddressList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data RecordRpzAaaaIpaddressListModel
...
func (l *RecordRpzAaaaIpaddressList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RecordRpzAaaaIpaddressListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_rpz_aaaa_ipaddress list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordRpzAaaaIpaddress
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordRpzAaaaIpaddress, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordRpzAaaaIpaddress, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordRpzAaaaIpaddress records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
