...
var _ datasource.DataSourceWithValidateConfig = &RecordRpzCnameDataSource{}
var _ datasource.DataSourceWithConfigure = &RecordRpzCnameDataSource{}
...
func (d *RecordRpzCnameDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_rpz_cname"
}

// RecordRpzCnameDataSourceModel is the filter model for the datasource
type RecordRpzCnameDataSourceModel struct {
...
func (m *RecordRpzCnameDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.RecordRpzCname, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: RecordRpzCnameAttrTypes})
		return
	}
	elements := make([]attr.Value, 0, len(from))
	for _, obj := range from {
		model := &RecordRpzCnameModel{}
		model.Flatten(ctx, obj, diags)
		objValue, d := types.ObjectValueFrom(ctx, RecordRpzCnameAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: RecordRpzCnameAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *RecordRpzCnameDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox RecordRpzCname from the NIOS backend.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(RecordRpzCnameResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *RecordRpzCnameDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewRecordRpzCnameService(d.backend, client.NIOS, client.UDDI)
}

func (d *RecordRpzCnameDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data RecordRpzCnameDataSourceModel
...
func (d *RecordRpzCnameDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.RecordRpzCname
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.RecordRpzCname, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.RecordRpzCname, error) {
			opts.Offset = offset
			opts.Limit = limit
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list RecordRpzCname records: %s", err))
		return
	}
