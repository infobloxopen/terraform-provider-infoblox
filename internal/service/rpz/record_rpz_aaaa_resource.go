...
var (
	_ resource.Resource                   = &RecordRpzAaaaResource{}
	_ resource.ResourceWithValidateConfig = &RecordRpzAaaaResource{}
	_ resource.ResourceWithConfigure      = &RecordRpzAaaaResource{}
	_ resource.ResourceWithImportState    = &RecordRpzAaaaResource{}
	_ resource.ResourceWithIdentity       = &RecordRpzAaaaResource{}
)
...
type RecordRpzAaaaResource struct {
	...
	service coresvc.RecordRpzAaaaService
}

func (r *RecordRpzAaaaResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_rpz_aaaa"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *RecordRpzAaaaResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *RecordRpzAaaaResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox RecordRpzAaaa in the NIOS backend.",
		Attributes:          RecordRpzAaaaResourceSchemaAttributes,
	}
}

func (r *RecordRpzAaaaResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewRecordRpzAaaaService(r.backend, client.NIOS, client.UDDI)
}

func (r *RecordRpzAaaaResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.RecordRpzAaaa](r.backend, op)
}

func (r *RecordRpzAaaaResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateRecordRpzAaaa(ctx, data, resp)
}
...
func (r *RecordRpzAaaaResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSRecordRpzAaaaModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSRecordRpzAaaaModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSRecordRpzAaaaAttrTypes, &resp.Diagnostics)
	}
	...
	if r.backend == core.BackendNIOS {
		ApplyRecordRpzAaaaNIOSUseFlags(ctx, req.Config, obj, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
	}
	...
	var (
		apiResp  *coremodel.RecordRpzAaaa
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: RecordRpzAaaaReturnFields,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create RecordRpzAaaa: %s", err))
		return
	}
...
func (r *RecordRpzAaaaResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read RecordRpzAaaa: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSRecordRpzAaaaModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read RecordRpzAaaa because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *RecordRpzAaaaResource) ReadByExtAttrs(ctx context.Context, data *RecordRpzAaaaModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.RecordRpzAaaa
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search RecordRpzAaaa by extattrs: %s", err))
		return true
	}
...
func (r *RecordRpzAaaaResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSRecordRpzAaaaModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSRecordRpzAaaaModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSRecordRpzAaaaModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSRecordRpzAaaaAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update RecordRpzAaaa: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSRecordRpzAaaaModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSRecordRpzAaaaAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *RecordRpzAaaaResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete RecordRpzAaaa: %s", err))
	}
}
