...
var (
	_ list.ListResource                   = &RecordRpzAaaaList{}
	_ list.ListResourceWithConfigure      = &RecordRpzAaaaList{}
	_ list.ListResourceWithValidateConfig = &RecordRpzAaaaList{}
)

func NewRecordRpzAaaaList() list.ListResource {
	return &RecordRpzAaaaList{}
}

type RecordRpzAaaaList struct {
	backend core.BackendType
	service coresvc.RecordRpzAaaaService
}

type RecordRpzAaaaListModel struct {
	Filters        types.Map `tfsdk:"filters"`
	ExtAttrFilters types.Map `tfsdk:"ext_attr_filters"`
}

func (l *RecordRpzAaaaList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_rpz_aaaa"
}

func (l *RecordRpzAaaaList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRecordRpzAaaaService(l.backend, client.NIOS, client.UDDI)
}

func (l *RecordRpzAaaaList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RecordRpzAaaa from the NIOS backend.",
...
func (l *RecordRpzAaaaList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data RecordRpzAaaaListModel
...
func (l *RecordRpzAaaaList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RecordRpzAaaaListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_record_rpz_aaaa list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RecordRpzAaaa
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RecordRpzAaaa, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RecordRpzAaaa, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RecordRpzAaaa records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
