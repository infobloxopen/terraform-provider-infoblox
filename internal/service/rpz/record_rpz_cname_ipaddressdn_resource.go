...
var (
	_ resource.Resource                   = &RecordRpzCnameIpaddressdnResource{}
	_ resource.ResourceWithValidateConfig = &RecordRpzCnameIpaddressdnResource{}
	_ resource.ResourceWithConfigure      = &RecordRpzCnameIpaddressdnResource{}
	_ resource.ResourceWithImportState    = &RecordRpzCnameIpaddressdnResource{}
	_ resource.ResourceWithIdentity       = &RecordRpzCnameIpaddressdnResource{}
)
...
type RecordRpzCnameIpaddressdnResource struct {
	...
	service coresvc.RecordRpzCnameIpaddressdnService
}

func (r *RecordRpzCnameIpaddressdnResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_record_rpz_cname_ipaddressdn"
	resp.ResourceBehavior = resource.ResourceBehavior{
		MutableIdentity: true,
	}
}

func (r *RecordRpzCnameIpaddressdnResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *RecordRpzCnameIpaddressdnResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox RecordRpzCnameIpaddressdn in the NIOS backend.",
		Attributes:          RecordRpzCnameIpaddressdnResourceSchemaAttributes,
	}
}

func (r *RecordRpzCnameIpaddressdnResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewRecordRpzCnameIpaddressdnService(r.backend, client.NIOS, client.UDDI)
}

func (r *RecordRpzCnameIpaddressdnResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.RecordRpzCnameIpaddressdn](r.backend, op)
}

func (r *RecordRpzCnameIpaddressdnResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
	...
	ValidateRecordRpzCnameIpaddressdn(ctx, data, resp)
}
...
func (r *RecordRpzCnameIpaddressdnResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSRecordRpzCnameIpaddressdnModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSRecordRpzCnameIpaddressdnModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSRecordRpzCnameIpaddressdnAttrTypes, &resp.Diagnostics)
	}
	...
	if r.backend == core.BackendNIOS {
		ApplyRecordRpzCnameIpaddressdnNIOSUseFlags(ctx, req.Config, obj, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
	}
	...
	var (
		apiResp  *coremodel.RecordRpzCnameIpaddressdn
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: RecordRpzCnameIpaddressdnReturnFields,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create RecordRpzCnameIpaddressdn: %s", err))
		return
	}
...
func (r *RecordRpzCnameIpaddressdnResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read RecordRpzCnameIpaddressdn: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSRecordRpzCnameIpaddressdnModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read RecordRpzCnameIpaddressdn because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *RecordRpzCnameIpaddressdnResource) ReadByExtAttrs(ctx context.Context, data *RecordRpzCnameIpaddressdnModel, resp *resource.ReadResponse) bool {
	...
	var (
		records  []*coremodel.RecordRpzCnameIpaddressdn
		httpResp *http.Response
	)
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search RecordRpzCnameIpaddressdn by extattrs: %s", err))
		return true
	}
...
func (r *RecordRpzCnameIpaddressdnResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSRecordRpzCnameIpaddressdnModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSRecordRpzCnameIpaddressdnModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSRecordRpzCnameIpaddressdnModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSRecordRpzCnameIpaddressdnAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update RecordRpzCnameIpaddressdn: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSRecordRpzCnameIpaddressdnModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSRecordRpzCnameIpaddressdnAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *RecordRpzCnameIpaddressdnResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete RecordRpzCnameIpaddressdn: %s", err))
	}
}
