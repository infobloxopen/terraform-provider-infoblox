...
var (
	_ list.ListResource                   = &InfraServiceList{}
	_ list.ListResourceWithConfigure      = &InfraServiceList{}
	_ list.ListResourceWithValidateConfig = &InfraServiceList{}
)

func NewInfraServiceList() list.ListResource {
	return &InfraServiceList{}
}

type InfraServiceList struct {
	backend core.BackendType
	service coresvc.InfraServiceService
}

type InfraServiceListModel struct {
	Filters    types.Map `tfsdk:"filters"`
	TagFilters types.Map `tfsdk:"tag_filters"`
}

func (l *InfraServiceList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_infra_service"
}

func (l *InfraServiceList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewInfraServiceService(l.backend, client.NIOS, client.UDDI)
}

func (l *InfraServiceList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox InfraService from the UDDI backend.",
...
func (l *InfraServiceList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data InfraServiceListModel
...
func (l *InfraServiceList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data InfraServiceListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_infra_service list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.InfraService
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.InfraService, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.InfraService, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list InfraService records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
