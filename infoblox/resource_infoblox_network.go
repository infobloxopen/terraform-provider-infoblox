...
func resourceNetworkCreate(d *schema.ResourceData, m interface{}, isIPv6 bool) error {
	...
	if !autoAllocateGateway && gateway != "none" {
		_, err = objMgr.AllocateIP(networkViewName, network.Cidr, gateway, isIPv6, ZeroMacAddr, "", "", nil, "MAC_ADDRESS", "", "", false, "", false, nil, false)
		if err != nil {
			return fmt.Errorf(
	...
	if isIPv6 {
		for i := 1; i <= reserveIPv6; i++ {
			...
			newAddr, err := objMgr.AllocateIP(
				networkViewName, network.Cidr, "", isIPv6, reservedDuid, "", "", nil, "MAC_ADDRESS", "", "", false, "", false, nil, false)
			if err != nil {
				return fmt.Errorf(
		...
		for i := 1; i <= reserveIPv4; i++ {
			newAddr, err := objMgr.AllocateIP(
				networkViewName, network.Cidr, "", isIPv6, ZeroMacAddr, "", "", nil, "MAC_ADDRESS", "", "", false, "", false, nil, false)
			if err != nil {
				return fmt.Errorf(
					"reservation in network block '%s' from network view '%s' failed: %s",
					network.Cidr, networkViewName, err.Error())
