...
- `allow_active_dir` (Attributes List) This field allows the zone to receive GSS-TSIG authenticated DDNS updates from DHCP clients and servers in an AD domain. Note that addresses specified in this field ignore the permission set in the st (see [below for nested schema](#nestedatt--results--nios--allow_active_dir))
...
- `ms_primaries` (Attributes List) The list with the Microsoft DNS servers that are primary servers for the zone. Although a zone typically has just one primary name server, you can specify up to ten independent servers for a single zo (see [below for nested schema](#nestedatt--results--nios--ms_primaries))
...
- `update_forwarding` (Attributes List) Use this field to allow or deny dynamic DNS updates that are forwarded from specific IPv4/IPv6 addresses, networks, or a named ACL. You can also provide TSIG keys for clients that are allowed or denie (see [below for nested schema](#nestedatt--results--nios--update_forwarding))
