# Go API client for Universal DDI Anycast API

Anycast capability enables HA (High Availability) configuration of Universal DDI applications that run on equipment located on customer's premises (on-prem hosts). Anycast supports DNS, as well as DNS-forwarding services.
...
```bash
go get github.com/infobloxopen/universal-ddi-go-client/anycast
```

Import the package into your code:
```go
import "github.com/infobloxopen/universal-ddi-go-client/anycast"
```
...
An API key is required to access Universal DDI Anycast API. You can obtain an API key by following the instructions in the guide for [Configuring User API Keys](https://docs.infoblox.com/space/BloxOneCloud/35430405/Configuring+User+API+Keys).

To use an API key with Universal DDI Anycast API, you can use the `option.WithAPIKey` option. For example:
