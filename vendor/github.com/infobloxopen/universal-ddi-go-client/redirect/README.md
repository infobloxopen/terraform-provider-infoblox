# Go API client for Infoblox Redirect API
...
```bash
go get github.com/infobloxopen/universal-ddi-go-client/redirect
```

Import the package into your code:
```go
import "github.com/infobloxopen/universal-ddi-go-client/redirect"
```
...
An API key is required to access Infoblox Redirect API. You can obtain an API key by following the instructions in the guide for [Configuring User API Keys](https://docs.infoblox.com/space/BloxOneCloud/35430405/Configuring+User+API+Keys).

To use an API key with Infoblox Redirect API, you can use the `option.WithAPIKey` option. For example:
