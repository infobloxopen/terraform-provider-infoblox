/*
Infoblox GRID API

OpenAPI specification for Infoblox NIOS WAPI GRID objects

API version: 2.13.6
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package grid

import (
	"encoding/json"
)

// checks if the MemberdnsattackmitigationDetectNxdomainResponses type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &MemberdnsattackmitigationDetectNxdomainResponses{}

// MemberdnsattackmitigationDetectNxdomainResponses struct for MemberdnsattackmitigationDetectNxdomainResponses
type MemberdnsattackmitigationDetectNxdomainResponses struct {
	// Determines if DNS attack detection is enabled or not.
	Enable *bool `json:"enable,omitempty"`
	// The high threshold value (in percentage) for starting DNS attack detection.
	High *int64 `json:"high,omitempty"`
	// The maximum number of events that have occurred before processing DNS attack detection.
	IntervalMax *int64 `json:"interval_max,omitempty"`
	// The minimum number of events that have occurred before processing DNS attack detection.
	IntervalMin *int64 `json:"interval_min,omitempty"`
	// The time interval between detection processing.
	IntervalTime *int64 `json:"interval_time,omitempty"`
	// The low threshold value (in percentage) for starting DNS attack detection.
	Low                  *int64 `json:"low,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _MemberdnsattackmitigationDetectNxdomainResponses MemberdnsattackmitigationDetectNxdomainResponses

// NewMemberdnsattackmitigationDetectNxdomainResponses instantiates a new MemberdnsattackmitigationDetectNxdomainResponses object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewMemberdnsattackmitigationDetectNxdomainResponses() *MemberdnsattackmitigationDetectNxdomainResponses {
	this := MemberdnsattackmitigationDetectNxdomainResponses{}
	return &this
}

// NewMemberdnsattackmitigationDetectNxdomainResponsesWithDefaults instantiates a new MemberdnsattackmitigationDetectNxdomainResponses object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewMemberdnsattackmitigationDetectNxdomainResponsesWithDefaults() *MemberdnsattackmitigationDetectNxdomainResponses {
	this := MemberdnsattackmitigationDetectNxdomainResponses{}
	return &this
}

// GetEnable returns the Enable field value if set, zero value otherwise.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetEnable() bool {
	if o == nil || IsNil(o.Enable) {
		var ret bool
		return ret
	}
	return *o.Enable
}

// GetEnableOk returns a tuple with the Enable field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetEnableOk() (*bool, bool) {
	if o == nil || IsNil(o.Enable) {
		return nil, false
	}
	return o.Enable, true
}

// HasEnable returns a boolean if a field has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) HasEnable() bool {
	if o != nil && !IsNil(o.Enable) {
		return true
	}

	return false
}

// SetEnable gets a reference to the given bool and assigns it to the Enable field.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) SetEnable(v bool) {
	o.Enable = &v
}

// GetHigh returns the High field value if set, zero value otherwise.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetHigh() int64 {
	if o == nil || IsNil(o.High) {
		var ret int64
		return ret
	}
	return *o.High
}

// GetHighOk returns a tuple with the High field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetHighOk() (*int64, bool) {
	if o == nil || IsNil(o.High) {
		return nil, false
	}
	return o.High, true
}

// HasHigh returns a boolean if a field has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) HasHigh() bool {
	if o != nil && !IsNil(o.High) {
		return true
	}

	return false
}

// SetHigh gets a reference to the given int64 and assigns it to the High field.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) SetHigh(v int64) {
	o.High = &v
}

// GetIntervalMax returns the IntervalMax field value if set, zero value otherwise.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetIntervalMax() int64 {
	if o == nil || IsNil(o.IntervalMax) {
		var ret int64
		return ret
	}
	return *o.IntervalMax
}

// GetIntervalMaxOk returns a tuple with the IntervalMax field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetIntervalMaxOk() (*int64, bool) {
	if o == nil || IsNil(o.IntervalMax) {
		return nil, false
	}
	return o.IntervalMax, true
}

// HasIntervalMax returns a boolean if a field has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) HasIntervalMax() bool {
	if o != nil && !IsNil(o.IntervalMax) {
		return true
	}

	return false
}

// SetIntervalMax gets a reference to the given int64 and assigns it to the IntervalMax field.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) SetIntervalMax(v int64) {
	o.IntervalMax = &v
}

// GetIntervalMin returns the IntervalMin field value if set, zero value otherwise.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetIntervalMin() int64 {
	if o == nil || IsNil(o.IntervalMin) {
		var ret int64
		return ret
	}
	return *o.IntervalMin
}

// GetIntervalMinOk returns a tuple with the IntervalMin field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetIntervalMinOk() (*int64, bool) {
	if o == nil || IsNil(o.IntervalMin) {
		return nil, false
	}
	return o.IntervalMin, true
}

// HasIntervalMin returns a boolean if a field has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) HasIntervalMin() bool {
	if o != nil && !IsNil(o.IntervalMin) {
		return true
	}

	return false
}

// SetIntervalMin gets a reference to the given int64 and assigns it to the IntervalMin field.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) SetIntervalMin(v int64) {
	o.IntervalMin = &v
}

// GetIntervalTime returns the IntervalTime field value if set, zero value otherwise.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetIntervalTime() int64 {
	if o == nil || IsNil(o.IntervalTime) {
		var ret int64
		return ret
	}
	return *o.IntervalTime
}

// GetIntervalTimeOk returns a tuple with the IntervalTime field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetIntervalTimeOk() (*int64, bool) {
	if o == nil || IsNil(o.IntervalTime) {
		return nil, false
	}
	return o.IntervalTime, true
}

// HasIntervalTime returns a boolean if a field has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) HasIntervalTime() bool {
	if o != nil && !IsNil(o.IntervalTime) {
		return true
	}

	return false
}

// SetIntervalTime gets a reference to the given int64 and assigns it to the IntervalTime field.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) SetIntervalTime(v int64) {
	o.IntervalTime = &v
}

// GetLow returns the Low field value if set, zero value otherwise.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetLow() int64 {
	if o == nil || IsNil(o.Low) {
		var ret int64
		return ret
	}
	return *o.Low
}

// GetLowOk returns a tuple with the Low field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) GetLowOk() (*int64, bool) {
	if o == nil || IsNil(o.Low) {
		return nil, false
	}
	return o.Low, true
}

// HasLow returns a boolean if a field has been set.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) HasLow() bool {
	if o != nil && !IsNil(o.Low) {
		return true
	}

	return false
}

// SetLow gets a reference to the given int64 and assigns it to the Low field.
func (o *MemberdnsattackmitigationDetectNxdomainResponses) SetLow(v int64) {
	o.Low = &v
}

func (o MemberdnsattackmitigationDetectNxdomainResponses) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o MemberdnsattackmitigationDetectNxdomainResponses) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Enable) {
		toSerialize["enable"] = o.Enable
	}
	if !IsNil(o.High) {
		toSerialize["high"] = o.High
	}
	if !IsNil(o.IntervalMax) {
		toSerialize["interval_max"] = o.IntervalMax
	}
	if !IsNil(o.IntervalMin) {
		toSerialize["interval_min"] = o.IntervalMin
	}
	if !IsNil(o.IntervalTime) {
		toSerialize["interval_time"] = o.IntervalTime
	}
	if !IsNil(o.Low) {
		toSerialize["low"] = o.Low
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *MemberdnsattackmitigationDetectNxdomainResponses) UnmarshalJSON(data []byte) (err error) {
	varMemberdnsattackmitigationDetectNxdomainResponses := _MemberdnsattackmitigationDetectNxdomainResponses{}

	err = json.Unmarshal(data, &varMemberdnsattackmitigationDetectNxdomainResponses)

	if err != nil {
		return err
	}

	*o = MemberdnsattackmitigationDetectNxdomainResponses(varMemberdnsattackmitigationDetectNxdomainResponses)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "enable")
		delete(additionalProperties, "high")
		delete(additionalProperties, "interval_max")
		delete(additionalProperties, "interval_min")
		delete(additionalProperties, "interval_time")
		delete(additionalProperties, "low")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableMemberdnsattackmitigationDetectNxdomainResponses struct {
	value *MemberdnsattackmitigationDetectNxdomainResponses
	isSet bool
}

func (v NullableMemberdnsattackmitigationDetectNxdomainResponses) Get() *MemberdnsattackmitigationDetectNxdomainResponses {
	return v.value
}

func (v *NullableMemberdnsattackmitigationDetectNxdomainResponses) Set(val *MemberdnsattackmitigationDetectNxdomainResponses) {
	v.value = val
	v.isSet = true
}

func (v NullableMemberdnsattackmitigationDetectNxdomainResponses) IsSet() bool {
	return v.isSet
}

func (v *NullableMemberdnsattackmitigationDetectNxdomainResponses) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableMemberdnsattackmitigationDetectNxdomainResponses(val *MemberdnsattackmitigationDetectNxdomainResponses) *NullableMemberdnsattackmitigationDetectNxdomainResponses {
	return &NullableMemberdnsattackmitigationDetectNxdomainResponses{value: val, isSet: true}
}

func (v NullableMemberdnsattackmitigationDetectNxdomainResponses) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableMemberdnsattackmitigationDetectNxdomainResponses) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
